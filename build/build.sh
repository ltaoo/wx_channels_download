...
build_windows() {
    ...
    CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -trimpath -tags "with_gvisor,embed_inject,sqlite_only,embed_frontend_inject" -ldflags="-s -w" -o "$OUTPUT_DIR/wx_video_download_windows_x86_64.exe"
...
build_macos() {
    echo "Building macOS..."
    CGO_ENABLED=1 GOOS=darwin SDKROOT=$(xcrun --sdk macosx --show-sdk-path) go build -trimpath -tags "embed_inject,sqlite_only,embed_frontend_inject" -ldflags="-s -w" -o "$OUTPUT_DIR/wx_video_download_macos"
    echo "Done: $OUTPUT_DIR/wx_video_download_macos"
}

build_macos_arm64() {
    echo "Building macOS arm64..."
    CGO_ENABLED=1 GOOS=darwin GOARCH=arm64 SDKROOT=$(xcrun --sdk macosx --show-sdk-path) go build -trimpath -tags "embed_inject,sqlite_only,embed_frontend_inject" -ldflags="-s -w" -o "$OUTPUT_DIR/wx_video_download_macos_arm64"
    echo "Done: $OUTPUT_DIR/wx_video_download_macos_arm64"
}

build_linux() {
    echo "Building Linux x86_64..."
    CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build -trimpath -tags "embed_inject,sqlite_only,embed_frontend_inject" -ldflags="-s -w" -o "$OUTPUT_DIR/wx_video_download_linux"
    echo "Done: $OUTPUT_DIR/wx_video_download_linux"
}

build_linux_arm64() {
    echo "Building Linux arm64..."
    CGO_ENABLED=1 GOOS=linux GOARCH=arm64 go build -trimpath -tags "embed_inject,sqlite_only,embed_frontend_inject" -ldflags="-s -w" -o "$OUTPUT_DIR/wx_video_download_linux_arm64"
