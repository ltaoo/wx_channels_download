...
function DashboardViewModel() {
  ...
  function normalize_overview(overview) {
    ...
    const hubs = overview.hubs.map((hub) => {
      ...
      const hub_online_count = clients.filter((client) => client.status === "online" || client.status === "busy").length;
      const hub_offline_count = clients.filter((client) => client.status === "offline").length;
      const hub_task_count = task_counts.reduce((total, item) => total + Number(item.count || 0), 0);
      const test_clients = clients.filter((client) =>
        (client.status === "online" || client.status === "busy") &&
        Array.isArray(client.capabilities) &&
        client.capabilities.includes("wxchannels.fetch")
      );
      const download_clients = clients.filter((client) =>
        (client.status === "online" || client.status === "busy") &&
        Array.isArray(client.capabilities) &&
      ...
      return Object.assign({}, hub, { clients, task_counts, test_clients, download_clients, online_count: hub_online_count, offline_count: hub_offline_count, task_count: hub_task_count });
    });
    return Object.assign({}, overview, {
      hubs,
      metrics: { hub_count: hubs.length, online_count, offline_count, task_count },
    });
  }
  ...
  function reconcile_test_drafts(hubs) {
    ...
    for (const hub of hubs) {
      ...
      const target_exists = clients.some((client) => client.client_id === current.target_client_id);
      test_drafts[hub.id] = {
        target_client_id: target_exists ? current.target_client_id : (clients[0] ? clients[0].client_id : ""),
        url: current.url || "",
      };
    }
    return test_drafts;
  ...
  function reconcile_download_drafts(hubs) {
    ...
    for (const hub of hubs) {
      const current = download_drafts[hub.id] || { target_client_id: "", download_dir: "", filename: "" };
      const clients = Array.isArray(hub.download_clients) ? hub.download_clients : [];
      const target_exists = clients.some((client) => client.client_id === current.target_client_id);
  ...
  function set_test_draft(hub_id, field, value) {
    const current = test_drafts_.value[hub_id] || { target_client_id: "", url: "" };
    test_drafts_.as(Object.assign({}, test_drafts_.value, {
      [hub_id]: Object.assign({}, current, { [field]: value }),
    }));
  }

  function set_download_draft(hub_id, field, value) {
    const current = download_drafts_.value[hub_id] || { target_client_id: "", download_dir: "", filename: "" };
    download_drafts_.as(Object.assign({}, download_drafts_.value, {
      [hub_id]: Object.assign({}, current, { [field]: value }),
    }));
  }
  ...
  function set_hub_test(hub_id, next_test) {
    ...
    tests_.as(Object.assign({}, tests_.value, {
      [hub_id]: Object.assign({}, current, next_test),
    }));
  }

  async function run_test(hub_id) {
    const draft = test_drafts_.value[hub_id] || { target_client_id: "", url: "" };
    if (!draft.target_client_id || !draft.url.trim()) {
      set_hub_test(hub_id, { submitting: false, task: null, error: "请选择在线电脑并填写视频号 URL" });
      return;
    }
    const old_timer = test_poll_timers.get(hub_id);
    if (old_timer !== undefined) clearTimeout(old_timer);
    set_hub_test(hub_id, { submitting: true, task: null, download: null, error: "" });
    try {
      const response = await fetch("/admin/api/tests", {
        ...
        body: JSON.stringify({ hub_id, target_client_id: draft.target_client_id, url: draft.url.trim() }),
      });
      const value = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(value.error || ("创建测试任务失败：HTTP " + response.status));
      if (!value.task || !value.task.id) throw new Error("创建测试任务返回格式不正确");
      set_hub_test(hub_id, { submitting: false, task: value.task, error: "" });
      schedule_test_poll(hub_id, value.task.id);
    } catch (error) {
      set_hub_test(hub_id, { submitting: false, task: null, error: error instanceof Error ? error.message : String(error) });
    }
  }
  ...
  function schedule_test_poll(hub_id, task_id) {
    ...
    if (current.task.status === "completed" || current.task.status === "failed") return;
    const old_timer = test_poll_timers.get(hub_id);
    if (old_timer !== undefined) clearTimeout(old_timer);
  ...
  async function refresh_test(hub_id, task_id) {
    ...
    if (!current || !current.task || current.task.id !== task_id) return;
    try {
      const response = await fetch("/admin/api/tasks/" + encodeURIComponent(task_id) + "?hub_id=" + encodeURIComponent(hub_id), {
        credentials: "same-origin",
        cache: "no-store",
      });
      const value = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(value.error || ("查询测试任务失败：HTTP " + response.status));
      if (!value.task || value.task.id !== task_id) throw new Error("测试任务查询返回格式不正确");
      set_hub_test(hub_id, { submitting: false, task: value.task, error: "" });
    } catch (error) {
      set_hub_test(hub_id, { error: error instanceof Error ? error.message : String(error) });
    }
  ...
  function set_download_test(hub_id, next_download) {
    ...
    set_hub_test(hub_id, { download: Object.assign({}, current_download, next_download) });
  }

  async function run_download_test(hub_id) {
    const current_test = tests_.value[hub_id];
    const source_task = current_test && current_test.task;
    const draft = download_drafts_.value[hub_id] || { target_client_id: "", download_dir: "", filename: "" };
    if (!source_task || source_task.status !== "completed") {
      set_download_test(hub_id, { submitting: false, task: null, error: "视频号内容任务尚未完成" });
      return;
    }
    if (!draft.target_client_id) {
      set_download_test(hub_id, { submitting: false, task: null, error: "请选择支持 download.create 的在线电脑" });
      return;
    }
    ...
    try {
      ...
      const value = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(value.error || ("创建下载任务失败：HTTP " + response.status));
      if (!value.task || !value.task.id) throw new Error("创建下载任务返回格式不正确");
      set_download_test(hub_id, { submitting: false, task: value.task, error: "" });
      schedule_download_poll(hub_id, value.task.id);
    } catch (error) {
      set_download_test(hub_id, { submitting: false, task: null, error: error instanceof Error ? error.message : String(error) });
    }
  }
  ...
  function schedule_download_poll(hub_id, task_id) {
    ...
    if (download.task.status === "completed" || download.task.status === "failed") return;
    const old_timer = download_poll_timers.get(hub_id);
    if (old_timer !== undefined) clearTimeout(old_timer);
    const timer = setTimeout(() => refresh_download_test(hub_id, task_id), 1500);
  ...
  async function refresh_download_test(hub_id, task_id) {
    ...
    try {
      ...
      const value = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(value.error || ("查询下载任务失败：HTTP " + response.status));
      if (!value.task || value.task.id !== task_id) throw new Error("下载任务查询返回格式不正确");
      set_download_test(hub_id, { submitting: false, task: value.task, error: "" });
    } catch (error) {
      set_download_test(hub_id, { error: error instanceof Error ? error.message : String(error) });
    }
  ...
  async function refresh() {
    ...
    loading_.as(true);
    error_.as("");
    refresh_promise = fetch("/admin/api/overview", { credentials: "same-origin", cache: "no-store" })
      .then(async (response) => {
        if (!response.ok) throw new Error("加载失败：HTTP " + response.status);
        const overview = await response.json();
        if (!overview || !Array.isArray(overview.hubs)) throw new Error("管理接口返回格式不正确");
        const normalized_overview = normalize_overview(overview);
        test_drafts_.as(reconcile_test_drafts(normalized_overview.hubs));
        download_drafts_.as(reconcile_download_drafts(normalized_overview.hubs));
    ...
    [
      ...
      View(
        ...
        [
          computed(
            props.metrics,
            (metrics) => String(Number(metrics[props.field] || 0)),
          ),
        ],
      ),
    ],
...
function task_meta_text(task) {
  if (!task) return "";
  let meta = "任务 " + String(task.id) +
    " · 执行电脑 " + String(task.assigned_client_id || task.target_client_id || "等待分配") +
    " · 尝试 " + String(task.attempt_count || 0) +
    " · 创建 " + format_time(task.created_at) +
    " · 更新 " + format_time(task.updated_at);
...
function TaskResultView(props) {
  ...
  const submitting_ = computed(props.result, (result) => Boolean(result.submitting));
  const status_text_ = computed(
    task_,
    (task) => task ? (props.status_labels[task.status] || String(task.status)) : "",
  );
  ...
  const output_visible_ = computed(
    task_,
    (task) => Boolean(task && (task.status === "completed" || task.status === "failed")),
  );
...
function DownloadPanelView(props) {
  ...
  const download_clients = Array.isArray(hub.download_clients)
    ...
    : [];
  const download_options = download_clients.length === 0
    ? [{ value: "", label: "没有在线且支持 download.create 的电脑" }]
    : download_clients.map((client) => ({
        value: String(client.client_id),
        label: String(client.client_id) +
          (client.status === "busy" ? "（正在执行任务）" : "（在线）"),
      }));
  const panel_visible_ = computed(
    props.test,
    (test) => Boolean(test.task && test.task.status === "completed"),
  );
  const download_ = computed(
    props.test,
    (test) => test.download || {},
  );
  const active_ = computed(
    ...
    (download) => Boolean(
      download.submitting ||
      (download.task && !["completed", "failed"].includes(download.task.status)),
    ),
  );
  ...
  const target_client_id_ = computed(
    props.draft,
    (draft) => String(draft.target_client_id || ""),
  );
  const download_dir_ = computed(
    props.draft,
    (draft) => String(draft.download_dir || ""),
  );
  const filename_ = computed(
    props.draft,
    (draft) => String(draft.filename || ""),
  );
  const submit_label_ = computed(
    active_,
    (active) => active ? "提交中…" : "提交下载任务",
  );
...
function TestPanelView(props) {
  ...
  const hub = props.hub;
  const hub_id = String(hub.id);
  const test_clients = Array.isArray(hub.test_clients)
    ? hub.test_clients
    : [];
  const test_options = test_clients.length === 0
  ...
  const url_ = computed(
    props.draft,
    (draft) => String(draft.url || ""),
  );
  const submit_label_ = computed(
    active_,
    (active) => active ? "测试中…" : "创建测试任务",
  );
...
function ClientView(props) {
  ...
  const status = client.status === "busy" || client.status === "online"
    ? client.status
    : "offline";
  const status_labels = {
    online: "在线",
  ...
  const time_text = status === "offline"
    ? "最近活跃 " + format_time(client.last_seen_at) +
      " · 离线于 " + format_time(client.disconnected_at)
    : "连接于 " + format_time(client.connected_at) +
      " · 最近活跃 " + format_time(client.last_seen_at);
...
function HubView(props) {
  ...
  const test_draft_ = computed(
    ...
    (drafts) => drafts[hub_id] ||
      { target_client_id: "", url: "" },
  );
  const download_draft_ = computed(
    vm$.state.download_drafts,
    (drafts) => drafts[hub_id] ||
      { target_client_id: "", download_dir: "", filename: "" },
  );
...
function ApplicationView() {
  ...
  const refresh_disabled_ = computed(
    vm$.state.loading,
