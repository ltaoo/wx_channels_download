...
export function SplitLayout(options) {
  ...
  for (let i = 0; i < panels.length; i++) {
    ...
    const content = p.useScroll
      ...
      : p.class
        ? [View({ dataset: { t: "shared-layout-split-layout-p-children" }, class: p.class }, p.children)]
        : p.children;
    elements.push(
      ResizablePanel(
        {
...
export function SidebarLayout(options, children) {
  ...
  return View({ dataset: { t: "shared-layout-sidebar-layout-flex" }, class: classNames(["w-full h-full", options.class]) }, [
    Flex({ class: "h-full" }, [
      View(
        {
          dataset: { t: "shared-layout-sidebar-layout-sidebar-value" },
          class: classNames([`w-[${sidebarWidth}]`, "h-full shrink-0", sidebarClass]),
        },
        sidebar,
      ),
      View({ dataset: { t: "shared-layout-sidebar-layout-row-children-value" }, class: "relative flex-1 w-0 h-full min-w-0" }, children),
    ]),
  ]);
}
...
export function StackLayout(options, children) {
  ...
  const elements = [];
  if (header) {
    elements.push(View({ dataset: { t: "shared-layout-stack-layout-header-value" }, class: classNames(["shrink-0", headerClass]) }, header));
  }
  elements.push(View({ dataset: { t: "shared-layout-stack-layout-row-children-value" }, class: "relative flex-1 min-h-0" }, children));
  if (footer) {
    elements.push(View({ dataset: { t: "shared-layout-stack-layout-footer-value" }, class: classNames(["shrink-0", footerClass]) }, footer));
  }
  return View(
    { dataset: { t: "shared-layout-stack-layout-elements-value" }, class: classNames(["w-full h-full flex flex-col", options.class]) },
    elements,
  );
}
