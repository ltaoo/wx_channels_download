(function (global) {
  ...
  function normalize_download_status_filter(status) {
    ...
    return typeof DOWNLOAD_TASK_STATUS_ORDER[value] === "number"
      ? value
      : "all";
  }
  ...
  function DownloaderPanelViewModel(props = {}) {
    ...
    const ensureTaskPage = (page) => {
      const normalizedPage = normalizePage(page);
      if (
        loaded_pages.has(normalizedPage) ||
        loading_pages.has(normalizedPage)
      ) {
        return;
      }
    ...
    const bindNativeListViewScroll = () => {
      ...
      if (
        !list_view_el ||
        typeof list_view_el.addEventListener !== "function"
      ) {
        return;
      }
    ...
    const methods = {
      ...
      async confirmCreateTaskFromPreview() {
        ...
        try {
          ...
          const filename =
            create_task_filename_.value || preview.task_name || "";
      ...
      async confirmCreatePlatformTaskFromPreview() {
        ...
        try {
          const platform =
            create_platform_text_.value || preview.platform || "";
          var content = {};
          try {
      ...
      upsert(task, options) {
        ...
        if (matchesFilter) {
          ...
          tasks_.as(
            sortTaskSlots(
              next.filter(
                (item) => !(isLoadedTask(item) && item.id === task.id),
              ),
              nextTotal,
            ),
          );
      ...
      async confirmOverwriteDownloadConflict() {
        ...
        try {
          if (action !== "skip") {
            ...
            const [err, data] =
              await methods.createDownloadTaskInDuplicate(body);
            if (err) {
              WXU.error({
        ...
        async onOk() {
          ...
          if (err) {
            WXU.log
              ...
              .Int("code", err.code || 0)
              .Msg(
                "overwriteConfirmDialog: retry creating download task failed",
              );
            WXU.error({
              msg: err.message || "创建下载任务失败",
