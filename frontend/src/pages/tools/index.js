...
export default function ToolsPageView(props) {
  ...
  function renderLogPanel() {
    return View({ class: "space-y-2 border border-zinc-200 dark:border-zinc-800 rounded-lg overflow-hidden" }, [
      // Header with toggle
      View({
        ...
        View({ class: "flex items-center gap-2" }, [
          View({ class: "text-sm font-medium text-zinc-700 dark:text-zinc-300" }, ["工作流日志"]),
          View({
        ...
        ok() {
          return View({
            ...
            Show({
              ...
              ok() {
                return View({ class: "text-xs text-zinc-400 py-4 text-center" }, ["等待日志..."]);
            ...
            For({ each: reqLogs_, render(entry) {
              ...
              return View({ class: "flex gap-2 text-xs" }, [
                // Timeline dot
                View({
                  style: `width:8px;height:8px;border-radius:50%;background:${color};margin-top:3px;flex-shrink:0;`,
                }),
                View({ class: "flex-1 min-w-0 space-y-0.5" }, [
                  View({ class: "flex items-center gap-2 flex-wrap" }, [
                    View({ class: "font-medium text-zinc-700 dark:text-zinc-300" }, [entry.phase || "info"]),
                    View({ class: "text-zinc-400 text-[10px]" }, [entry.timestamp || ""]),
                  ]),
                  View({ class: "text-zinc-600 dark:text-zinc-400" }, [entry.message || ""]),
                  hasData ? View({
                    ...
                    ok() {
                      ...
                      const rows = Object.keys(data).map(function (k) {
                        ...
                        return View({ class: "flex gap-2" }, [
                          View({ class: "text-zinc-500 font-mono flex-shrink-0" }, [k + ":"]),
                          View({ class: "text-zinc-700 dark:text-zinc-300 font-mono break-all" }, [str]),
                        ]);
                      });
                      return View({ class: "mt-1 p-2 rounded bg-zinc-50 dark:bg-zinc-900 space-y-0.5" }, rows);
                    },
                  }),
                ]),
              ]);
  ...
  return ScrollView({
    ...
    View({ class: "p-6 max-w-3xl space-y-6" }, [
      // ---- Header ----
      View({ class: "text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50" }, ["工具箱"]),

      // ---- Tab Bar ----
      View({ class: "flex border-b border-zinc-200 dark:border-zinc-800" }, [
      ...
      Show({
        ...
        ok() {
          return View({ class: "space-y-4 pt-2" }, [
            // URL input
            View({ class: "space-y-1.5" }, [
              View({ class: "text-sm font-medium text-zinc-700 dark:text-zinc-300" }, ["URL"]),
              Input({
            ...
            // Fetch mode selector
            View({ class: "space-y-1.5" }, [
              View({ class: "text-sm font-medium text-zinc-700 dark:text-zinc-300" }, ["请求方式"]),
              View({ class: "flex gap-2" }, FETCH_MODES.map((m) => {
            ...
            (() => {
              ...
              return View({ class: "space-y-2" }, [
                ...
                Show({
                  when: open_,
                  ok() {
                    return View({ class: "space-y-3 p-3 border rounded-lg border-zinc-200 dark:border-zinc-800" }, [
                      View({ class: "text-xs font-medium text-zinc-500" }, ["自定义 Headers（每行 key: value）"]),
                      Textarea({
                        store: new Timeless.ui.InputCore({
                      ...
                      View({ class: "grid grid-cols-3 gap-3" }, [
                        ...
                        View({ class: "space-y-1" }, [
                          View({ class: "text-xs text-zinc-500" }, ["文件名"]),
                          Input({
                        store: new Timeless.ui.InputCore({
            ...
            Show({
              ...
              ok() {
                return View({ class: "p-3 rounded-md border border-red-200 bg-red-50 text-sm text-red-700 dark:border-red-800 dark:bg-red-950 dark:text-red-300" }, [
            ...
            Show({
              ...
              ok() {
                return View({ class: "space-y-2 p-3 rounded-lg border border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-950" }, [
                  View({ class: "text-sm font-medium text-green-800 dark:text-green-200" }, ["保存完成"]),
                  View({ class: "text-xs text-green-700 dark:text-green-300 space-y-1" }, [
                    View({}, [computed(reqResult_, (v) => `路径: ${v?.FilePath || v?.file_path || "-"}`)]),
                    View({}, [computed(reqResult_, (v) => `大小: ${v?.FileSize != null ? formatBytes(v.FileSize) : "-"}`)]),
                    View({}, [computed(reqResult_, (v) => `资源数: ${v?.ResourceCount != null ? v.ResourceCount : "-"}`)]),
                    View({}, [computed(reqResult_, (v) => {
                      const d = v?.Duration;
                      if (d == null) return `耗时: -`;
      ...
      Show({
        ...
        ok() {
          return View({ class: "space-y-6 pt-2" }, [
            // Daemon name + status
            View({ class: "flex items-center gap-3 flex-wrap" }, [
              View({ class: "text-sm font-medium text-zinc-700 dark:text-zinc-300" }, ["Daemon:"]),
              Input({
                store: new Timeless.ui.InputCore({
                  value: dmName_.value,
                  placeholder: "default",
            ...
            Show({
              ...
              ok() {
                return View({ class: "space-y-2" }, [
                  View({ class: "text-sm font-medium text-zinc-700 dark:text-zinc-300" }, ["浏览器标签页"]),
                  View({ class: "space-y-1" }, [
                    For({ each: dmTabs_, render(tab, idx) {
                      return View({ class: "text-xs text-zinc-600 dark:text-zinc-400 p-2 rounded bg-zinc-50 dark:bg-zinc-900" }, [
            ...
            Show({
              ...
              ok() {
                return View({ class: "space-y-2" }, [
                  View({ class: "text-sm font-medium text-zinc-700 dark:text-zinc-300" }, ["当前页面信息"]),
                  View({
            ...
            // ---- Remote Daemon ----
            View({ class: "pt-4 border-t border-zinc-200 dark:border-zinc-800 space-y-3" }, [
              View({ class: "text-sm font-semibold text-zinc-700 dark:text-zinc-300" }, ["远程 Daemon"]),
              View({ class: "grid grid-cols-3 gap-3" }, [
                View({ class: "space-y-1" }, [
                  View({ class: "text-xs text-zinc-500" }, ["Profile Name"]),
                  Input({
                    store: new Timeless.ui.InputCore({
                ...
                View({ class: "space-y-1" }, [
                  View({ class: "text-xs text-zinc-500" }, ["Proxy Country (如 de)"]),
                  Input({
                    store: new Timeless.ui.InputCore({
                ...
                View({ class: "space-y-1" }, [
                  View({ class: "text-xs text-zinc-500" }, ["Timeout (秒)"]),
                  Input({
                    store: new Timeless.ui.InputCore({
      ...
      Show({
        ...
        ok() {
          return View({ class: "space-y-4 pt-2" }, [
            ...
            Show({
              when: appLoading_,
              ok() { return View({ class: "text-sm text-zinc-500" }, ["加载中..."]); },
            }),
            ...
            Show({
              ...
              ok() {
                return View({ class: "space-y-3" }, [
                  View({ class: "text-sm font-medium text-zinc-700 dark:text-zinc-300" }, [
                  ...
                  For({ each: appList_, render(app) {
                    ...
                    return View({
                      ...
                      View({ class: "flex items-center justify-between" }, [
                        View({ class: "flex items-center gap-2" }, [
                          View({ class: "text-sm font-medium text-zinc-900 dark:text-zinc-100" }, [name]),
                          version ? View({ class: "text-xs text-zinc-400" }, [`v${version}`]) : null,
                          installed
                            ? View({ class: "inline-flex px-1.5 py-0.5 rounded text-xs bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300" }, ["已安装"])
                            : null,
                        ]),
                      ...
                      desc ? View({ class: "text-xs text-zinc-500 dark:text-zinc-400" }, [desc]) : null,
                    ]);
                  } }),
                ]);
              },
            ...
            Show({
              ...
              ok() {
                return View({ class: "space-y-2 pt-2" }, [
                  View({ class: "text-sm font-medium text-zinc-700 dark:text-zinc-300" }, ["任务列表"]),
                  For({ each: appTasks_, render(task) {
                    ...
                    return View({
                      ...
                      View({ class: "font-mono text-zinc-500" }, [tid]),
                      View({ class: "text-zinc-700 dark:text-zinc-300" }, [action]),
                      View({ class: "text-zinc-600 dark:text-zinc-400" }, [appName]),
                      View({ class: status === "completed" ? "text-green-600" : status === "failed" ? "text-red-600" : "text-zinc-500" }, [status]),
                    ]);
                  } }),
                ]);
              },
