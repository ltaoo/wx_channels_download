...
function FormRender(props, children) {
  ...
  return View({ dataset: { t: "settings-page-form-render-field-names-list-fragment" }, class: Timeless.classNames([props.class, "space-y-5"]) }, [
    For({
      each: fieldNames,
      render(name) {
        ...
        return View(
          {
            dataset: { t: "settings-page-form-render-match" },
          ...
          [
            ...
            Show({
              when: inline,
              ok() {
                return View({ dataset: { t: "settings-page-form-render-stack-field-inline-label-description-value" }, class: "min-w-0 space-y-1 pt-0.5" }, [
                  FieldInlineLabel({ for: fid, store: field$ }),
                  description
                    ? View(
                        {
                          dataset: { t: "settings-page-form-render-description-value-text" },
                          class:
            ...
            Show({
              ...
              ok() {
                return View(
                  {
                    dataset: { t: "settings-page-form-render-description-value-text-2" },
                    class:
                      "text-xs leading-5 text-zinc-500 dark:text-zinc-400",
                  },
                  [description],
...
function SettingsGroup(group) {
  return View(
    {
      dataset: { t: "settings-page-settings-group-card-group-name-项配置-group-count-form-render" },
      class:
    ...
    [
      View(
        {
          dataset: { t: "settings-page-settings-group-row-group-name-项配置-group-count" },
          class:
            "mb-5 flex flex-wrap items-center justify-between gap-3 border-b border-zinc-100 pb-4 dark:border-zinc-900",
        },
        [
          View({ dataset: { t: "settings-page-settings-group-group-name-项配置-group-count" } }, [
            View(
              {
                dataset: { t: "settings-page-settings-group-group-name-heading" },
                class:
                  "text-base font-semibold text-zinc-950 dark:text-zinc-50",
              },
              [group.name],
            ),
            View({ dataset: { t: "settings-page-settings-group-项配置-group-count-text" }, class: "mt-1 text-xs text-zinc-500 dark:text-zinc-400" }, [
              `${group.count} 项配置`,
            ]),
          ]),
        ],
...
export default function SettingsPageView(props) {
  ...
  return View(
    {
      dataset: { t: "settings-page-page-root-row-设置-path-value-config-yaml-button-button-scroll-view" },
    ...
    [
      View(
        {
          dataset: { t: "settings-page-header-设置-path-value-config-yaml-button-button" },
          class:
            "border-b border-zinc-200 bg-white px-6 py-5 dark:border-zinc-800 dark:bg-zinc-950",
        },
        [
          View({ dataset: { t: "settings-page-row-设置-path-value-config-yaml-button-button" }, class: "flex flex-wrap items-center justify-between gap-3" }, [
            View({ dataset: { t: "settings-page-设置-path-value-config-yaml" }, class: "min-w-0" }, [
              View(
                {
                  dataset: { t: "settings-page-设置-heading" },
                  class:
              ...
              View(
                {
                  dataset: { t: "settings-page-path-value-config-yaml-text" },
                  class:
            ...
            View({ dataset: { t: "settings-page-row-button-button" }, class: "flex items-center gap-2" }, [
              Button(
                {
      ...
      ScrollView({ store: vm$.ui.view, class: "flex-1" }, [
        View({ dataset: { t: "settings-page-scroll-content-config-groups-region" }, class: "p-6" }, [
          Show({
            when: vm$.state.error,
            ok() {
              return View(
                {
                  dataset: { t: "settings-page-error-card-vm-state-error-text" },
                  class:
          ...
          Show({
            ...
            ok() {
              return View(
                {
                  dataset: { t: "settings-page-row-icon-loader-加载中" },
                  class:
              ...
              return View({ dataset: { t: "settings-page-grid-vm-state-forms-list" }, class: "grid gap-5 xl:grid-cols-2" }, [
                For({
                  each: vm$.state.forms,
                  render(group) {
