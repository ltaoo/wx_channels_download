...
import {
  ...
  uninstallProxyCertificateByName,
  updateProxyConfig,
} from "@/biz/request.js";
    ...
    [
      View(
        ...
        [
          View(
            {
              dataset: { t: "proxy-page-section-panel-title-row" },
            ...
            [
              ...
              View(
                {
                  dataset: { t: "proxy-page-section-panel-title-text" },
                  class: "min-w-0",
                },
                [
                  View(
          ...
          props.actions
            ? View(
                {
                  dataset: { t: "proxy-page-section-panel-actions" },
...
function InfoItem(props) {
  const { label, value, icon, tone = "neutral" } = props;
  const tone$ = isRefLike(tone) ? tone : ref(tone);
  return View(
    {
...
function FieldBlock(label, control, hint) {
  return View(
    { dataset: { t: "proxy-page-field-block" }, class: "min-w-0 space-y-2" },
    [
      Label(
        { class: "block text-xs font-medium text-zinc-600 dark:text-zinc-300" },
        [label],
      ),
      control,
      hint
...
function SwitchField(label, switchStore, value_, icon) {
  return View(
    ...
    [
      View(
        {
          dataset: { t: "proxy-page-switch-field-label-row" },
        ...
        [
          ...
          View(
            {
              dataset: { t: "proxy-page-switch-field-label" },
              class: "truncate text-sm text-zinc-800 dark:text-zinc-100",
            },
            [label],
          ),
          View(
...
function ProxyPageModel(props) {
  const reqs = {
    ...
    update: new Timeless.RequestCore(updateProxyConfig, {
      client: api_client$,
    }),
    restart: new Timeless.RequestCore(restartProxyService, {
      client: api_client$,
    }),
    start: new Timeless.RequestCore(startService, { client: api_client$ }),
    stop: new Timeless.RequestCore(stopService, { client: api_client$ }),
    enableSystem: new Timeless.RequestCore(enableSystemProxy, {
      client: api_client$,
    }),
    disableSystem: new Timeless.RequestCore(disableSystemProxy, {
      client: api_client$,
    }),
    generateCert: new Timeless.RequestCore(generateProxyCertificate, {
      client: api_client$,
    }),
    installCert: new Timeless.RequestCore(installProxyCertificate, {
      client: api_client$,
    }),
    uninstallCert: new Timeless.RequestCore(uninstallProxyCertificate, {
      client: api_client$,
    }),
    uninstallCertByName: new Timeless.RequestCore(
      uninstallProxyCertificateByName,
      { client: api_client$ },
    ),
  };
  ...
  ui.skipCertSwitch.onChange((value) =>
    state.form.skipInstallRootCert.as(!!value),
  );
  ...
  function syncForm(data) {
    ...
    state.form.certName.as(
      cert.name || data?.certificate?.name || "wx_channels_download",
    );
  ...
  function buildValues() {
    return {
      ...
      "proxy.tcpRelay.hostname":
        state.form.tcpRelayHostname.value.trim() || "127.0.0.1",
  ...
  const methods = {
    ...
    uninstallCertByName(name) {
      // Snapshot before optimistic update for error rollback
      const snapshot = state.status.value;
      const certs = snapshot?.certificate?.all_certificates;
      if (certs) {
        state.status.as({
          ...snapshot,
          certificate: {
            ...snapshot.certificate,
            all_certificates: certs.map((cert) =>
              cert.name === name
                ? { ...cert, installed: false, trusted: false }
                : cert,
            ),
          },
        });
      ...
      return run(
        () => reqs.uninstallCertByName.run({ name }),
        `证书 "${name}" 已从系统中移除`,
      ).then((data) => {
        if (!data && certs) {
          // Revert optimistic update on failure
          state.status.as(snapshot);
        }
      });
    },
    downloadPEM() {
...
function ServicePanel(vm$) {
  ...
  return SectionPanel(
    ...
    [
      View(
        {
          dataset: { t: "proxy-page-service-summary-grid" },
          class: "grid gap-3 md:grid-cols-3",
        },
        [
          InfoItem(
            "状态",
            computed(status_, serviceStatusText),
            "activity",
            computed(status_, statusTone),
          ),
          InfoItem(
            "监听",
            computed(service_, (s) => (s.listening ? "端口可用" : "未监听")),
            "network",
            computed(service_, (s) => (s.listening ? "success" : "danger")),
          ),
          InfoItem(
            "地址",
            computed(service_, (s) => s.addr || "-"),
            "link",
          ),
        ],
      ),
      View(
        {
          dataset: { t: "proxy-page-service-actions" },
...
function ConfigPanel(vm$) {
  return SectionPanel(
    ...
    [
      View(
        {
          dataset: { t: "proxy-page-config-form-grid" },
      ...
      View(
        {
          dataset: { t: "proxy-page-config-switches" },
        ...
        [
          ...
          SwitchField(
            "跳过根证书安装",
            vm$.ui.skipCertSwitch,
            vm$.state.form.skipInstallRootCert,
            "shield-off",
          ),
        ],
      ),
      View(
        ...
        [
          View(
            {
              dataset: { t: "proxy-page-tcp-relay-row" },
              class:
                "grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,160px)]",
            },
            [
              SwitchField(
                "TCP Relay",
                vm$.ui.tcpRelaySwitch,
                vm$.state.form.tcpRelayEnabled,
                "cable",
              ),
              FieldBlock(
                "Relay 主机",
                Input({ store: vm$.ui.tcpRelayHostname }),
              ),
              FieldBlock(
                "Relay 端口",
                NumberInput({ store: vm$.ui.tcpRelayPort }),
              ),
            ],
          ),
        ],
...
function SystemProxyPanel(vm$) {
  ...
  return SectionPanel(
    ...
    [
      View(
        {
          dataset: { t: "proxy-page-system-proxy-grid" },
          class: "grid gap-3 md:grid-cols-3",
        },
        [
          InfoItem(
            "配置",
            computed(system_, (s) => boolText(s.configured)),
            "settings",
          ),
          InfoItem(
            "当前",
            computed(system_, (s) => (s.enabled ? "已设置" : "未设置")),
            "power",
            computed(system_, (s) => (s.enabled ? "success" : "neutral")),
          ),
          InfoItem(
            "匹配",
            computed(system_, (s) => (s.matched ? "匹配代理端口" : "不匹配")),
            "check-circle",
            computed(system_, (s) => (s.matched ? "success" : "warning")),
          ),
        ],
      ),
      Show({
      ...
      View(
        {
          dataset: { t: "proxy-page-system-proxy-actions" },
          class: "mt-4 flex flex-wrap gap-2",
        },
        [
          Timeless.weui.Button(
...
function certSourceLabel(source) {
  if (source === "sunny_net") return "SunnyNet (内置)";
  if (source === "mitmproxy") return "mitmproxy (第三方)";
  if (source === "configured") return "用户配置";
  if (source === "generated") return "本机生成";
  return source || "未知";
}
  ...
  return SectionPanel(
    {
      ...
      actions: [
        Timeless.weui.Button(
          ...
          [Icon({ name: "badge-check", size: 15 }), "安装根证书"],
        ),
        Timeless.weui.Button(
          {
            store: new Timeless.ui.ButtonCore({
    ...
    [
      ...
      Show({
        when: computed(
          cert_,
          (c) => !c.all_certificates || c.all_certificates.length === 0,
        ),
        ok() {
          return View(
            {
              dataset: { t: "proxy-page-certificate-empty" },
          ...
          return View(
            {
              dataset: { t: "proxy-page-certificate-cards" },
              class: "space-y-3",
            },
            [
              For({
                each: computed(cert_, (t) => {
                  return t.all_certificates || [];
                }),
                render(entry) {
                  return View(
                    {
                      dataset: {
                        t: "proxy-page-certificate-card",
                        active: entry.is_active ? "true" : "false",
                      },
                      class: cn([
                        "rounded-lg border p-4",
                        computed(entry.is_active, (active) =>
                          active
                    ...
                    [
                      View(
                        {
                          dataset: { t: "proxy-page-certificate-card-header" },
                        ...
                        [
                          View(
                            {
                              dataset: {
                                t: "proxy-page-certificate-card-title",
                              },
                              class: "min-w-0",
                            },
                            [
                              View(
                                {
                                  dataset: {
                                    t: "proxy-page-certificate-card-name-row",
                                  },
                                ...
                                [
                                  View(
                                    {
                                      dataset: {
                                        t: "proxy-page-certificate-card-name",
                                      },
                                      class:
                                        "truncate text-base font-semibold text-zinc-900 dark:text-zinc-100",
                                    },
                                    [entry.name || "-"],
                                  ),
                                  Show({
                                    ...
                                    ok() {
                                      return View(
                                        {
                                          dataset: {
                                            t: "proxy-page-certificate-card-active-badge",
                                          },
                                          class:
                                            "inline-flex h-5 items-center rounded-full bg-emerald-100 px-2 text-xs font-medium text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300",
                                        },
                                        ["当前使用"],
                                      );
                                    },
                              ...
                              View(
                                {
                                  dataset: {
                                    t: "proxy-page-certificate-card-source",
                                  },
                                  class:
                                    "mt-0.5 text-xs text-zinc-500 dark:text-zinc-400",
                                },
                                [certSourceLabel(entry.source)],
                              ),
                            ],
                          ),
                          Show({
                            ...
                            ok() {
                              return Timeless.weui.Button(
                                {
                                  store: new Timeless.ui.ButtonCore({
                                    variant: "outline",
                                    size: "sm",
                                    onClick() {
                                      vm$.methods.uninstallCertByName(
                                        entry.name,
                                      );
                                    },
                                  }),
                      ...
                      View(
                        {
                          dataset: { t: "proxy-page-certificate-card-grid" },
                          class: "mt-3 grid gap-3 sm:grid-cols-2",
                        },
                        [
                          InfoItem({
                            label: "信任状态",
                            value: computed(entry, (e) => {
                              if (!e.installed) return "未安装";
                              if (e.trusted) return "已安装并信任";
                              return "已安装，未信任";
                            }),
                            icon: "badge-check",
                            tone: computed(entry, (e) => {
                              if (e.trusted) return "success";
                              if (e.installed) return "warning";
                              return "neutral";
                            }),
                          }),
                          InfoItem({
                            label: "有效期至",
                            value: computed(entry, (e) =>
                              e.detail && e.detail.not_after
                                ? String(e.detail.not_after).slice(0, 10)
                                : "-",
                            ),
                            icon: "calendar",
                          }),
                      ...
                      Show({
                        when: computed(
                          entry,
                          (e) => e.risk_warnings && e.risk_warnings.length > 0,
                        ),
                        ok() {
                          return View(
                            {
                              dataset: {
                                t: "proxy-page-certificate-card-risks",
                              },
                              class:
                                "mt-3 rounded-md border border-red-200 bg-red-50 p-3 dark:border-red-900 dark:bg-red-950/30",
                            },
                            [
                              For({
                                each: computed(entry, (t) => {
                                  return t.risk_warnings || [];
                                }),
                                render(msg) {
                                  return View(
                                    {
                                      dataset: {
                                        t: "proxy-page-certificate-card-risk-item",
                                      },
                                      class:
                                        "flex items-start gap-1.5 text-xs text-red-700 dark:text-red-300",
                                    },
                                    [
                                      Icon({
                                        name: "alert-triangle",
                                      ...
                                      View(
                                        {
                                          dataset: {
                                            t: "proxy-page-certificate-card-risk-text",
                                          },
                                          class: "min-w-0",
                                        },
                                        [msg],
    ...
    [
      View(
        ...
        [
          View(
            {
              dataset: { t: "proxy-page-header-row" },
              class: "flex flex-wrap items-center justify-between gap-3",
            },
            [
              View(
                {
                  dataset: { t: "proxy-page-header-title-block" },
                  class: "min-w-0",
                },
                [
                  View(
              ...
              View(
                {
                  dataset: { t: "proxy-page-header-actions" },
