...
function appendSpeedSample(previousTask, nextTask, timestamp = Date.now()) {
  const existing = Array.isArray(previousTask?.speed_history)
    ? previousTask.speed_history
    : [];
  const speed = Number(nextTask?.progress_info?.speed || 0);
  const status = nextTask?.status;
  const previousStatus = previousTask?.status;
  const last = existing[existing.length - 1];

  if (!Number.isFinite(speed) || speed < 0) return existing;

  // A resumed task reports speed=0 until its first 500 ms sampling window is
  // complete. That value means "not sampled yet", so do not create a fake dip.
  if (status === DownloadTaskStatus.Downloading && speed === 0 && !last) {
    return existing;
  }

  // Record one terminal zero when an active task stops, but do not keep adding
  // zeroes while paused/finished records are refreshed.
  if (status !== DownloadTaskStatus.Downloading) {
    if (
      previousStatus === DownloadTaskStatus.Downloading &&
      last &&
      last.speed > 0
    ) {
      return [...existing, { timestamp, speed: 0 }].slice(-SPEED_HISTORY_LIMIT);
    }
    return existing;
  }

  if (last && timestamp - last.timestamp < SPEED_SAMPLE_INTERVAL_MS) {
    if (last.speed === speed) return existing;
    // WebSocket updates inside the current sampling window move only the live
    // chart head. The timestamp stays fixed so the next window still creates a
    // new, settled point instead of postponing it indefinitely.
    return [...existing.slice(0, -1), { ...last, speed }];
  }

  return [...existing, { timestamp, speed }].slice(-SPEED_HISTORY_LIMIT);
}
...
export function normalizeTask(task) {
  // V1: Parse config_json to get platform metadata
  ...
  return {
    ...
    speed_history: Array.isArray(task.speed_history) ? task.speed_history : [],
    percent,
...
export function DownloadTaskPanelModel(props) {
  ...
  const speed_history_by_task_ = new Map();
  ...
  function normalizeCounts(data, list) {
    ...
    // V1: Count each status from the list
    return {
      total,
  ...
  function mergeRuntimeTask(rawTask) {
    ...
    if (idx >= 0) {
      ...
      const merged = trackTaskSpeed(
        ...
        normalizeTask({
          ...
          engine_task_id:
            nextTask.engine_task_id || prevTask.engine_task_id,
    ...
    const trackedTask = trackTaskSpeed(null, nextTask);
    updateCounts(null, trackedTask.status, true);
    if (!matches) return;
    tasks_.as(sortTasks([trackedTask, ...current]));
  ...
  function removeRuntimeTask(rawTask) {
    ...
    if (found) {
      speed_history_by_task_.delete(taskStateKey(found));
  ...
  function downloadTaskID(task) {
    // V1: task.id is the database primary key
  ...
  function trackTaskSpeed(previousTask, nextTask, sample = true) {
    const key = taskStateKey(nextTask) || taskStateKey(previousTask);
    const stored = key ? speed_history_by_task_.get(key) : null;
    const previous = {
      ...(previousTask || {}),
      speed_history:
        stored || previousTask?.speed_history || nextTask?.speed_history || [],
    };
    const speedHistory = sample
      ? appendSpeedSample(previous, nextTask)
      : previous.speed_history;
    if (key) speed_history_by_task_.set(key, speedHistory);
    return { ...nextTask, speed_history: speedHistory };
  }
    ...
    if (message.type === "task_stats") {
      ...
      if (stats) {
        status_counts_.as({
          ...
          running: Number(stats.downloading || 0),
          queued: Number(stats.waiting || 0),
          done: Number(stats.finished || 0),
          error: Number(stats.error || 0),
          paused: Number(stats.paused || 0),
        });
        total_.as(Number(stats.total || 0));
      }
      return;
    }
  ...
  async function load(page = 1) {
    ...
    const list = (r.data.list || []).map((task) =>
      trackTaskSpeed(null, normalizeTask(task), false),
    );
