...
const SPEED_CHART_HEIGHT = 96;
const SPEED_CHART_PADDING = 6;
const SPEED_CHART_POINT_LIMIT = 60;

function speedChartMetrics(task) {
  const history = (Array.isArray(task?.speed_history)
    ? task.speed_history
    : []
  ).filter((sample) => Number.isFinite(Number(sample?.speed)));
  const maxSpeed = Math.max(1, ...history.map((sample) => Number(sample.speed)));
  const innerHeight = SPEED_CHART_HEIGHT - SPEED_CHART_PADDING * 2;
  const pointStep = SPEED_CHART_WIDTH / (SPEED_CHART_POINT_LIMIT - 1);
  const points = history.map((sample, index) => {
    // Each sample owns a fixed horizontal slot. New samples advance the live
    // head one slot; existing points no longer stretch across the chart again.
    const x = index * pointStep;
    const y =
      SPEED_CHART_PADDING +
      innerHeight * (1 - Math.max(0, Number(sample.speed)) / maxSpeed);
    return { x, y };
  });
  ...
  const areaPath = linePath
    ? `${linePath} L${lastPoint.x.toFixed(1)},${SPEED_CHART_HEIGHT} L0,${SPEED_CHART_HEIGHT} Z`
    : "";
  return { history, maxSpeed, linePath, areaPath, lastPoint };
}

function mountedSVGElement(event) {
  ...
  return typeof SVGElement !== "undefined" && target.$elm instanceof SVGElement
    ? target.$elm
    : null;
}

function SpeedHistoryChart(task) {
  return View(
    {
      dataset: { t: "home-downloads-page-speed-history-chart" },
      class:
        "download-speed-chart rounded-md border border-zinc-100 bg-white/70 px-3 py-2 dark:border-zinc-800 dark:bg-zinc-950/60",
    },
    [
      View(
        {
          class:
            "mb-1.5 flex items-center justify-between gap-3 text-[11px] text-zinc-400 dark:text-zinc-500",
        },
        [
          ...
          View({ class: "tabular-nums" }, [
            computed(task, (t) => {
              const metrics = speedChartMetrics(t);
              return metrics.history.length
                ? `峰值 ${formatBytes(metrics.maxSpeed)}/s`
                : "等待速度样本";
            }),
          ]),
        ],
      ...
      Show({
        when: computed(
          task,
          (t) => speedChartMetrics(t).history.length > 0,
        ),
        ok() {
          return SVG.SVG(
            {
              class: "download-speed-chart__svg block h-20 w-full",
              onMounted(event) {
                const element = mountedSVGElement(event);
                if (!element) return;
                // Timeless 0.28 does not forward generic SVG attributes such as
                // viewBox/stroke/fill, so set structural attributes explicitly.
                element.setAttribute(
                  "viewBox",
                  `0 0 ${SPEED_CHART_WIDTH} ${SPEED_CHART_HEIGHT}`,
                );
            ...
            [
              SVG.Path({
                d: `M0,${SPEED_CHART_HEIGHT / 2} H${SPEED_CHART_WIDTH}`,
                class: "download-speed-chart__grid download-speed-chart__grid--dash",
              }),
              SVG.Path({
                d: `M0,${SPEED_CHART_HEIGHT - 1} H${SPEED_CHART_WIDTH}`,
                class: "download-speed-chart__grid",
              }),
              SVG.Path({
                d: computed(task, (t) => speedChartMetrics(t).areaPath),
                class: "download-speed-chart__area",
              }),
              SVG.Path({
                d: computed(task, (t) => speedChartMetrics(t).linePath),
                class: "download-speed-chart__line",
              }),
              SVG.Circle({
                cx: computed(
                  task,
                  (t) => speedChartMetrics(t).lastPoint?.x ?? 0,
                ),
                cy: computed(
                  task,
                  (t) => speedChartMetrics(t).lastPoint?.y ?? 0,
                ),
                r: 3.5,
                class: "download-speed-chart__head",
              }),
            ],
          );
        },
          ...
          return View(
            {
              class:
                "flex h-20 items-center justify-center text-xs text-zinc-400 dark:text-zinc-600",
            },
            ["首个有效采样产生后显示趋势"],
          );
        },
...
function inferContentType(platform, content) {
  ...
  if (platform === "douyin" || platform === "wxchannels") {
  ...
  return View(
    ...
    [
      Show({
        when: computed(
          data_,
          (content) => normalizePlatformID(content.platform) === "wxchannels",
        ),
        ok() {
