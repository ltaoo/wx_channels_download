...
- `getCookies(options)`: 通过 background 的 `chrome.cookies` 读取 cookie，默认读取当前页面 URL 可用的 cookie。
- `getCookieHeader(options)`: 获取可直接放入 HTTP `Cookie` 请求头的字符串。
- `getCookiesByDomain(options)`: 按 cookie domain 分组返回 cookie。
...
扩展参考 CookieCloud 的实现方式，在 `background.js` 里使用 `chrome.cookies.getAll` 获取 cookie。`manifest.json` 已声明 `cookies` 权限和 `<all_urls>` host 权限。

默认调用只返回当前页面 URL 可用的 cookie：

```js
var result = await WXExt.getCookies();
console.log(result.cookies, result.header);
```

按域名读取：

```js
var zhihuCookie = await WXExt.getCookieHeader({
  domains: ["zhihu.com"],
});
```
...
```js
var grouped = await WXExt.getCookiesByDomain({
  domains: ["zhihu.com", "zhuanlan.zhihu.com"],
  blacklist: ["analytics.example"],
});
```

需要显式读取扩展可访问的全部 cookie 时传 `all: true`：

```js
var allCookies = await WXExt.getCookies({ all: true });
```
