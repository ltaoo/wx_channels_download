...
const PlatformID = "youku"

type ProfileFetcher interface {
	FetchProfileWithSeasons(ctx context.Context, rawURL string) (*youkupkg.ProfileWithSeasons, error)
}
...
func (h *Handler) Match(rawURL string) bool {
	return matchHost(rawURL, "youku.com")
}
...
func (h *Handler) Probe(ctx context.Context, input contentdownload.ProbeInput) (*contentdownload.Probe, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("fetch youku profile: %w", err)
	}
	if profile == nil {
		return nil, fmt.Errorf("fetch youku profile: empty profile")
	}
	contentID := jsonartifact.FirstNonEmpty(profile.ID, input.URL)
