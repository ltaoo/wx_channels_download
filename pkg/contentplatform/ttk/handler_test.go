...
func (fakeFetcher) FetchNovelChapters(url string) (*Novel, error) {
	return &Novel{
		...
		Author: "author",
		Chapters: []Chapter{
			{Index: 1, Title: "chapter 1", URL: "https://ttks.tw/chapter/1.html"},
		},
	}, nil
}
...
func TestMatchProbeResolveNovel(t *testing.T) {
	h := New(fakeFetcher{})
	if !h.Match("https://ttks.tw/book/sample") {
		t.Fatal("expected ttk novel match")
	}
	probe, err := h.Probe(context.Background(), contentdownload.ProbeInput{URL: "https://ttks.tw/book/sample"})
	if err != nil {
		t.Fatalf("Probe: %v", err)
	}
	if contentdownload.ContentType(probe.Content) != "novel" || probe.ContentID != "sample" {
...
func TestProbeChapter(t *testing.T) {
	h := New(fakeFetcher{})
	probe, err := h.Probe(context.Background(), contentdownload.ProbeInput{URL: "https://ttks.tw/chapter/1.html"})
	if err != nil {
		t.Fatalf("Probe: %v", err)
	}
