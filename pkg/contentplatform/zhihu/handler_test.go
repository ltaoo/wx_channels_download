...
func (fakePageFetcher) FetchAnswerPage(rawURL string) (*zhihupkg.AnswerPage, error) {
	return &zhihupkg.AnswerPage{
		...
		Answer: zhihupkg.Answer{
			...
			Excerpt: "excerpt",
			Author:  zhihupkg.User{Name: "author", URLToken: "answer-author", AvatarURL: "https://example.com/answer-avatar.jpg"},
		},
	}, nil
}
...
func (fakePageFetcher) FetchQuestionPage(rawURL string) (*zhihupkg.QuestionPage, error) {
	return &zhihupkg.QuestionPage{
		...
		Question: zhihupkg.Question{
			...
			Author:  zhihupkg.User{Name: "question author", URLToken: "question-author", AvatarURL: "https://example.com/question-avatar.jpg"},
		},
	}, nil
}
...
func (fakePageFetcher) FetchArticlePage(rawURL string) (*zhihupkg.ArticlePage, error) {
	return &zhihupkg.ArticlePage{
		...
		Article: zhihupkg.Article{
			...
			Author:   zhihupkg.User{Name: "article author", URLToken: "article-author", AvatarURL: "https://example.com/article-avatar.jpg"},
		},
	}, nil
}
...
func TestProbeAnswerReturnsBodyOutput(t *testing.T) {
	...
	if output["author_avatar_url"] != "https://example.com/answer-avatar.jpg" ||
		output["author_homepage_url"] != "https://www.zhihu.com/people/answer-author" {
		t.Fatalf("answer output author fields = %#v", output)
	}
	metadata := contentdownload.ContentMetadataOf(probe.Content)
	if metadata["author_avatar_url"] != "https://example.com/answer-avatar.jpg" ||
		metadata["author_homepage_url"] != "https://www.zhihu.com/people/answer-author" {
		t.Fatalf("answer metadata author fields = %#v", metadata)
	}
}
...
func TestProbeQuestionAndArticleContentFields(t *testing.T) {
	...
	if contentdownload.ContentCoverURL(question.Content) != "" {
		t.Fatalf("question cover = %q", contentdownload.ContentCoverURL(question.Content))
	}
	...
	if output := contentdownload.ContentOutputOf(question.Content); output["body_html"] != "<p>question body</p>" ||
		output["author_avatar_url"] != "https://example.com/question-avatar.jpg" ||
		output["author_homepage_url"] != "https://www.zhihu.com/people/question-author" {
	...
	if output := contentdownload.ContentOutputOf(article.Content); output["body_html"] != "<p>article body</p>" ||
		output["author_avatar_url"] != "https://example.com/article-avatar.jpg" ||
		output["author_homepage_url"] != "https://www.zhihu.com/people/article-author" {
