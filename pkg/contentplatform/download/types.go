...
type ResolvedRequest struct {
	...
	// Files describes the expected output file tree. Empty means the request
	// produces a single file at the task destination path.
	...
	// Internal stores executor-only data reused from Probe/Resolve. It may
	// contain large objects and is never serialized.
	Internal map[string]any `json:"-"`
}
...
type DownloadSpec struct {
	...
	// Protocol selects the SourceExecutor, such as http, multi_http, zip, zhihu,
	// or officialaccount.
	Protocol string `json:"protocol,omitempty"`
	// Connections is the requested parallel connection count when supported.
	Connections int `json:"connections,omitempty"`
	// ChunkSize requests sequential Range downloads in fixed-size chunks.
	ChunkSize int64 `json:"chunk_size,omitempty"`
}

// MultiSourceSpec describes one concrete source inside a multi-stream download.
// It is intentionally HTTP-shaped because the current multi executor mirrors
// yt-dlp's video-only + audio-only direct URL flow.
...
type ContentSummary struct {
	...
	// AuthorHomepageURL is the author's profile or platform homepage URL.
...
func NewContent[T any](summary ContentSummary, data T, metadata map[string]any, output map[string]any) *Content[T] {
	summary, defaultedAuthor := NormalizeContentSummary(summary)
	if defaultedAuthor {
		if homepage := firstNonEmpty(contentMapHomepage(metadata), contentMapHomepage(output)); homepage != "" {
			summary.AuthorHomepageURL = homepage
		} else {
			metadata = withDefaultAuthorHomepage(metadata, summary.AuthorHomepageURL)
			output = withDefaultAuthorHomepage(output, summary.AuthorHomepageURL)
		}
	}
	return &Content[T]{
		Summary:  summary,
...
func ContentSummaryOf(content any) ContentSummary {
	...
	if envelope, ok := content.(contentEnvelope); ok {
		summary, _ := NormalizeContentSummary(envelope.ContentSummary())
		return summary
	}
	return ContentSummary{}
}
...
func ContentAuthorHomepageURL(content any) string {
	summary := ContentSummaryOf(content)
	return firstNonEmpty(
		metadataString(ContentMetadataOf(content),
			"author_homepage_url",
			"author_url",
			"profile_url",
			"channel_url",
		...
		metadataString(ContentOutputOf(content),
			"author_homepage_url",
			"author_url",
			"profile_url",
			"channel_url",
...
func NormalizeContentSummary(summary ContentSummary) (ContentSummary, bool) {
	displayAuthor := firstNonEmpty(summary.Author, summary.AuthorNickname)
	defaultedAuthor := false
	if displayAuthor == "" {
		displayAuthor = scraper.DefaultAuthor(summary.Platform)
		defaultedAuthor = displayAuthor != ""
	}
	if summary.Author == "" {
		summary.Author = displayAuthor
	}
	if summary.AuthorNickname == "" {
		summary.AuthorNickname = displayAuthor
	}
	if defaultedAuthor && strings.TrimSpace(summary.AuthorHomepageURL) == "" {
		summary.AuthorHomepageURL = scraper.HomepageURL(summary.Platform)
	}
	return summary, defaultedAuthor
}

func withDefaultAuthorHomepage(values map[string]any, homepage string) map[string]any {
	homepage = strings.TrimSpace(homepage)
	if homepage == "" || contentMapHomepage(values) != "" {
		return values
	}
	...
	out["author_homepage_url"] = homepage
	return out
}

func contentMapHomepage(values map[string]any) string {
	return metadataString(values,
		"author_homepage_url",
		"author_url",
		"profile_url",
		"channel_url",
