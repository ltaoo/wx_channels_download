...
const (
	...
	FileNodeStatusPending     = "pending"
	FileNodeStatusDownloading = "downloading"
	FileNodeStatusDone        = "done"
	FileNodeStatusError       = "error"
)
...
func NewDownloader(router *Router, downloadDir string, opts ...DownloaderOption) *Downloader {
	...
	d.executors = append(d.executors, NewHTTPExecutor(nil), NewMultiHTTPExecutor(nil), NewZipExecutor(nil), NewInlineHTMLExecutor(), NewInlineJSONExecutor())
...
func (d *Downloader) Create(ctx context.Context, input ResolveInput) (*Task, error) {
	...
	task.Files = CloneFileNodes(resolved.Files)
...
func (d *Downloader) CreateResolved(resolved *ResolvedRequest) (*Task, error) {
	...
	task := &Task{
		...
		Files:     CloneFileNodes(resolved.Files),
		CreatedAt: now,
		UpdatedAt: now,
	}
	d.store(task)
...
func (d *Downloader) Start(ctx context.Context, taskID string) error {
	...
	err := executor.Execute(ctx, ExecuteRequest{
		...
		OnFiles: func(files []FileNode) {
			d.update(task, func(t *Task) {
				t.Files = CloneFileNodes(files)
			})
			d.emit(EventTaskProgress, task)
		},
	})
	...
	d.update(task, func(t *Task) {
		...
		if len(t.Files) == 0 {
			t.Files = SingleFileNodes(t.FilePath, t.Progress.TotalBytes, FileNodeStatusDone)
		}
...
func (d *Downloader) MarkPaused(taskID string) *Task {
	task := d.GetTask(taskID)
	if task == nil {
		return nil
	}
