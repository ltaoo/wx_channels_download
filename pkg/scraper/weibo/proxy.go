...
const imageProxyReferer = BaseURL + "/"
...
func newImageProxyRequest(ctx context.Context, targetURL string, inbound *http.Request) (*http.Request, error) {
	...
	if parsed.Scheme != "http" && parsed.Scheme != "https" {
		return nil, fmt.Errorf("unsupported weibo image URL scheme: %s", parsed.Scheme)
	}
	if !isAllowedImageProxyHost(parsed.Hostname()) {
		return nil, fmt.Errorf("unsupported weibo image host: %s", parsed.Hostname())
	}
...
func isAllowedImageProxyHost(host string) bool {
	...
	return host == "sinaimg.cn" ||
		strings.HasSuffix(host, ".sinaimg.cn") ||
		host == "sinaimg.com" ||
		strings.HasSuffix(host, ".sinaimg.com")
}
