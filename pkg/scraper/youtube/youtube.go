...
func (c *Client) Resolve(ctx context.Context, input contentdownload.ResolveInput) (*contentdownload.ResolvedRequest, error) {
	...
	resolved := &contentdownload.ResolvedRequest{
		...
		Filename:     filename,
		Suffix:       suffix,
		Download:     youtubeDownloadSpec(variant, info.WebpageURL, downloadURL, headers),
...
func (c *Client) Extract(ctx context.Context, rawURL string) (*VideoInfo, error) {
	...
	if apiKey := stringFromMap(ytcfg, "INNERTUBE_API_KEY"); apiKey != "" {
		for _, client := range c.requestedInnertubeClients() {
			...
			apiResponse, apiErr := c.fetchPlayerAPIForClient(ctx, videoID, ytcfg, watchURL, client, player.signatureTimestamp())
			if apiErr != nil {
...
func (c *Client) fetchPlayerAPIForClient(ctx context.Context, videoID string, ytcfg map[string]any, referer string, client innertubeClient, sts string) (rawPlayerResponse, error) {
	apiKey := stringFromMap(ytcfg, "INNERTUBE_API_KEY")
	if apiKey == "" {
...
func (r *playerResolver) solveSignature(challenge string) (string, error) {
	...
	if name == "" {
		return "", fmt.Errorf("signature function not found")
	}
	return solvePlayerFunction(code, name, challenge)
}
...
func (r *playerResolver) solveN(challenge string) (string, error) {
	...
	if name == "" {
		return "", fmt.Errorf("n function not found")
	}
	return solvePlayerFunction(code, name, challenge)
}
