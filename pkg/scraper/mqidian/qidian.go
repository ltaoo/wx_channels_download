...
func DefaultUserAgent() string {
	return "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148"
}
...
func (c *Client) FetchBookProfileContext(ctx context.Context, id string) (*BookProfile, error) {
	...
	return c.fetchBookProfileURL(ctx, reqURL)
}

func (c *Client) FetchBookCatalogContext(ctx context.Context, id string) (*BookProfile, error) {
	reqURL := strings.TrimRight(firstNonEmpty(c.BaseURL, BaseURL), "/") + "/book/" + strings.TrimSpace(id) + "/catalog/"
	return c.fetchBookProfileURL(ctx, reqURL)
}

func (c *Client) fetchBookProfileURL(ctx context.Context, reqURL string) (*BookProfile, error) {
...
func bookProfileFromPageContext(reqURL string, root *PageContextRoot) *BookProfile {
	...
	bookID := int64String(firstNonZero(info.BookID, pageData.BookID))
	if bookID == "" {
		if parts, ok := ParseURL(reqURL); ok {
			bookID = parts.BookID
		}
	}
	authorID := int64String(firstNonZero(info.AuthorID, pageData.AuthorInfo.AuthorID.Int64()))
	...
	profile := &BookProfile{
		...
		Title:            firstNonEmpty(info.BookName, pageData.BookName),
		Description:      htmlFragmentText(info.Desc),
		Slogan:           firstNonEmpty(info.ActionStatus, pageData.ActionStatus, info.SignStatus),
		LatestUpdateAt:   qidianUpdateTime(info.UpdTimes, firstNonEmpty(firstRecentUpdateTime(pageData.RecentChapters), pageData.UpdateTime)),
		Tags:             qidianTags(pageData),
		ChapterCount:     firstNonZeroInt(pageData.ChapterTotalCnt, pageData.ChapterCount, countCatalogChapters(pageData.CatalogVolumes), len(pageData.RecentChapters)),
		WordCount:        info.WordsCnt,
		DisplayWordCount: info.ShowWordsCnt,
		Category:         info.ChanName,
		SubCategory:      info.SubCateName,
		Status:           firstNonEmpty(info.BookStatus, pageData.BookStatus, info.ActionStatus, pageData.ActionStatus),
		Author: Author{
			ID:     authorID,
			Name:   authorName,
	...
	if len(pageData.CatalogVolumes) > 0 {
		profile.Volumes = catalogVolumesToBookVolumes(bookID, pageData.CatalogVolumes)
		if latest := lastChapter(profile.Volumes); profile.LatestChapter.Title == "" {
	...
	if len(profile.Volumes) == 0 && len(pageData.RecentChapters) > 0 {
...
func catalogVolumesToBookVolumes(bookID string, volumes []CatalogVolume) []BookVolume {
	var out []BookVolume
	chapterIdx := 0
	for i, src := range volumes {
		volume := BookVolume{Idx: i + 1, Title: cleanText(src.Name)}
		for _, chapter := range src.Chapters {
			chapterIdx++
			volume.Chapters = append(volume.Chapters, catalogChapterToChapter(bookID, chapterIdx, chapter))
		}
...
func catalogChapterToChapter(bookID string, idx int, item CatalogChapter) Chapter {
	...
	return Chapter{
		...
		Title:       item.Name,
		URL:         chapterURL,
		Locked:      item.FreeStatus == 0,
		WordCount:   item.WordCount,
		PublishedAt: parseQidianLocalTime(item.UpdateTime),
	}
}
...
func recentChapterToChapter(bookID string, idx int, item RecentChapter) Chapter {
	...
	return Chapter{
		...
		URL:         chapterURL,
		WordCount:   item.WordCount,
		PublishedAt: parseQidianLocalTime(firstNonEmpty(item.UpdateTime2, item.UpdateTime)),
	}
}

func countCatalogChapters(volumes []CatalogVolume) int {
	total := 0
...
func qidianUpdateTime(unixSeconds int64, text string) time.Time {
	...
	return parseQidianLocalTime(text)
}

func parseQidianLocalTime(text string) time.Time {
