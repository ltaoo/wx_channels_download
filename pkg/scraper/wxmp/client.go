...
// Fetch the push message list for an official account
func (c *OfficialAccountClient) HandleFetchMsgList(ctx *gin.Context) {
	...
	logger := c.logger.With().
		...
		Int("offset", _offset).
		Logger()
	// If the page provides full credential info, use it directly instead of looking up the local account
	uin := ctx.Query("uin")
	key := ctx.Query("key")
...
func (c *OfficialAccountClient) HandleFetchArticleList(ctx *gin.Context) {
	...
	if err != nil {
		code := result.CodeFetchMsgFailed
		msg := "fetch article failed"
		if c, m, _, ok := codedErrorOf(err); ok {
			code = c
			msg = m
		}
...
// On the local frontend, manually refresh specified official accounts (supports batch)
...
func (c *OfficialAccountClient) HandleOfficialAccountProxy(ctx *gin.Context) {
	...
	if strings.Contains(contentType, "text/html") {
		...
		if err != nil {
			// If reading body fails, at least attempt to return the status code (may be too late but try not to crash)
			return
		}
		bodyString := string(bodyBytes)
		// Use regex to replace mmbiz_jpg/png/gif links with proxy links
		// Pattern: https://mmbiz.qpic.cn/mmbiz_xxx/ or https://mmbiz.qpic.cn/sz_mmbiz_xxx/ followed by non-quote, non-whitespace chars
		// Compatible with both http and https, different image format suffixes and path prefixes
...
func (c *OfficialAccountClient) BuildMsgListReferer(acct *OfficialAccount) string {
	params := url.Values{}
...
func (c *OfficialAccountClient) fetchArticleList(biz string) (*ArticleListResponse, error) {
	...
	if strings.TrimSpace(existing.AuthorId) == "" {
		return nil, newCodedError(result.CodeInvalidParams, "缺少 author_id", nil)
	}
	if strings.TrimSpace(existing.AppmsgToken) == "" {
		return nil, newCodedError(result.CodeInvalidParams, "缺少 appmsg_token", nil)
	}
...
// FetchAllMsgURLs loops to fetch all push message article links for an official account
