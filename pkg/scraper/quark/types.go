// Package quark fetches public Quark Drive share pages.
package quark
...
const PlatformID = "quark"

// File is one entry in a Quark Drive share tree.
type File struct {
	Fid           string `json:"fid"`
...
// DownloadableFiles returns pointers to every file in the share tree.
func (s *Share) DownloadableFiles() []*File {
	if s == nil {
		return nil
	}
	files := make([]*File, 0)
	...
	walk = func(entries []File) {
		for index := range entries {
			if entries[index].IsDir {
				walk(entries[index].Children)
				continue
			}
			files = append(files, &entries[index])
		}
	}
	walk(s.Files)
	return files
...
// ParseShareURL validates a Quark Drive share URL and returns its password ID.
func ParseShareURL(raw_url string) (string, error) {
	parsed_url, err := url.Parse(strings.TrimSpace(raw_url))
	if err != nil {
		return "", fmt.Errorf("解析夸克网盘 URL 失败: %w", err)
	}
	if parsed_url.Scheme != "http" && parsed_url.Scheme != "https" {
		return "", fmt.Errorf("夸克网盘仅支持 HTTP/HTTPS URL")
	}
	if strings.ToLower(parsed_url.Hostname()) != "pan.quark.cn" {
		return "", fmt.Errorf("不是夸克网盘分享 URL")
	}
