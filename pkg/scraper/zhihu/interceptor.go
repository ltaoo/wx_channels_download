...
func NewInterceptorPlugins(cfg InterceptorConfig, logger *zerolog.Logger) []*echo.Plugin {
	...
	plugin := &echo.Plugin{
		...
		OnResponse: func(ctx *echo.Context) {
			...
			frontend.AppendScripts(&early_injected, "", url_build("/public/timeless/0.33.0/timeless.umd.min.js", version_query))
			frontend.AppendStylesheets(&early_injected, "", url_build("/public/timeless/0.33.0/timeless.weui.css", version_query))
			frontend.AppendScripts(&early_injected, "", url_build("/public/timeless/0.33.0/timeless.weui.umd.min.js", version_query))
			frontend.AppendScripts(&early_injected, "", url_build("/public/timeless/0.33.0/timeless.dom.umd.min.js", version_query))
			frontend.AppendScripts(&early_injected, "", url_build("/public/timeless/0.33.0/timeless.web.umd.min.js", version_query))
