...
func (c *Client) doBytes(method, rawURL, referer string) ([]byte, error) {
	return c.doBytesWithHTTP(method, rawURL, referer)
}

func (c *Client) doBytesWithClawreq(method, rawURL, referer string) ([]byte, error) {
	client := c.clawreqClient
	if client == nil {
...
func (c *Client) doBytesWithHTTP(method, rawURL, referer string) ([]byte, error) {
...
func debugSnippet(body []byte) string {
	if len(body) <= 256 {
		return string(body)
	}
...
func (c *Client) doImageBytes(rawURL string, referer string) ([]byte, string, error) {
	if c.clawreqClient != nil {
		return c.doImageBytesWithClawreq(rawURL, referer)
	}
	return c.doImageBytesWithHTTP(rawURL, referer)
}

func (c *Client) doImageBytesWithClawreq(rawURL string, referer string) ([]byte, string, error) {
	client := c.clawreqClient
	if client == nil {
		return nil, "", fmt.Errorf("zhihu: clawreq client not initialized")
	}
...
func (c *Client) doImageBytesWithHTTP(rawURL string, referer string) ([]byte, string, error) {
...
func (c *Client) doAPIBytes(endpoint, referer string) ([]byte, error) {
	...
	if c.clawreqClient != nil {
		return c.doAPIBytesWithClawreq(endpoint, referer)
	}
	return c.doAPIBytesWithHTTP(endpoint, referer)
}

func (c *Client) doAPIBytesWithClawreq(endpoint, referer string) ([]byte, error) {
	client := c.clawreqClient
	if client == nil {
		return nil, fmt.Errorf("zhihu: clawreq client not initialized")
	}
