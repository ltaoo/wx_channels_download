...
(() => {
   ...
   */
  function insert_download_btn($parent) {
    var $btn = Icons.download_btn2();
    WXU.attach_download_dropdown_menu($btn);
    $btn.onclick = WXU.downloadBtnHandler;
    $parent.appendChild($btn);
  }
  async function insert_download_btn_to_old_feed_profile_page() {
    const $btn = Icons.download_btn1();
    WXU.attach_download_dropdown_menu($btn);
    $btn.onclick = WXU.downloadBtnHandler;
  ...
  async function insert_download_btn_to_feed_profile_page() {
    // console.log(
    ...
    // console.log('before insert_download_btn', $operations_parents);
    for (let i = 0; i < $operations_parents.length; i += 1) {
      insert_download_btn($operations_parents[i]);
    }
  }
  WXU.observe_node(
    ".slides-item",
    function () {
      insert_download_btn_to_feed_profile_page();
    },
    function () {
      render_sider_tools();
    },
