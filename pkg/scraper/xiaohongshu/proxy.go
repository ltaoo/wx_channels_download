...
func newImageProxyRequest(ctx context.Context, targetURL string, inbound *http.Request) (*http.Request, error) {
	...
	if parsed.Scheme != "http" && parsed.Scheme != "https" {
		return nil, fmt.Errorf("unsupported xiaohongshu image URL scheme: %s", parsed.Scheme)
	}
	if !isAllowedImageProxyHost(parsed.Hostname()) {
		return nil, fmt.Errorf("unsupported xiaohongshu image host: %s", parsed.Hostname())
	}
...
func isAllowedImageProxyHost(host string) bool {
	...
	return host == "xhscdn.com" ||
		strings.HasSuffix(host, ".xhscdn.com") ||
		host == "xhscdn.net" ||
		strings.HasSuffix(host, ".xhscdn.net")
}
