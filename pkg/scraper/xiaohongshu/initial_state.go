...
func ExtractInitialStateJSON(body []byte) (json.RawMessage, error) {
	...
	if idx < 0 {
		return nil, fmt.Errorf("missing xiaohongshu initial state")
	}
	rest := body[idx+len(initialStateMarker):]
	eq := bytes.IndexByte(rest, '=')
	if eq < 0 {
		return nil, fmt.Errorf("missing xiaohongshu initial state assignment")
	}
	rest = rest[eq+1:]
	open := bytes.IndexByte(rest, '{')
	if open < 0 {
		return nil, fmt.Errorf("missing xiaohongshu initial state object")
	}
	...
	if !json.Valid(jsonBytes) {
		...
		if err := json.Unmarshal(jsonBytes, &payload); err != nil {
			return nil, fmt.Errorf("invalid xiaohongshu initial state json: %w", err)
		}
		return nil, fmt.Errorf("invalid xiaohongshu initial state json")
	}
...
func extractBalancedObject(input []byte) ([]byte, error) {
	...
	for i, b := range input {
		...
		case '}':
			...
			if depth < 0 {
				return nil, fmt.Errorf("invalid xiaohongshu initial state object")
			}
		}
	}
	return nil, fmt.Errorf("unterminated xiaohongshu initial state object")
}
