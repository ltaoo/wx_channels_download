...
func NewClient(client HTTPClient) *Client {
	...
	if client == nil {
		c.HTMLFetcher = NewClawreqFetcher()
	}
	return c
}
...
func NewHTTPClientWithOptions(client HTTPClient, cookie, userAgent string) *Client {
	return &Client{HTTPClient: client, Cookie: cookie, UserAgent: userAgent}
}
...
func (c *Client) FetchChapterContent(rawURL string) (chapter *ChapterContent, err error) {
	...
	result, err := sessionClient.fetchChapter(rawURL)
	if err != nil {
		return nil, err
	}
...
func (c *Client) fetchChapter(rawURL string) (*ChapterFetchResult, error) {
	...
	htmlText, err := c.fetch(parts.Canonical, BaseURL+"/book/"+parts.BookID+"/")
	if err != nil {
		return nil, err
	}
	...
	for i := 0; i < concurrency; i++ {
		...
		go func() {
			...
			for index := range jobs {
				...
				item.URL = firstNonEmpty(item.URL, chapter.URL)
				if item.Content != nil {
					item.Content.Title = firstNonEmpty(item.Content.Title, chapter.Title)
...
func (c *Client) fetch(rawURL string, referer string) (string, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		if resp.StatusCode == http.StatusForbidden {
			return "", fmt.Errorf("69shuba HTTP 403: request was blocked before browser rendering; configure 69shuba.fetcher=clawreq, cdp, or sandbox for protected pages: %s", strings.TrimSpace(string(body)))
		}
