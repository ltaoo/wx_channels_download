...
func TestParseNovelHTMLPrefersFullCatalogListOverBookmark(t *testing.T) {
	html := `<!doctype html><html><head><meta charset="utf-8"><meta property="og:novel:book_name" content="book"></head><body>
<div class="catalog"><ul><li data-num="7"><a id="bookcase" href="/txt/1/99">latest bookmark</a></li></ul></div>
<div class="catalog" id="catalog"><ul>
<li data-num="1"><a href="/txt/1/1">chapter 1</a></li>
<li data-num="2"><a href="/txt/1/2">chapter 2</a></li>
</ul></div>
</body></html>`
	novel, err := ParseNovelHTML("https://www.69shuba.com/book/1/", html)
	if err != nil {
		t.Fatalf("ParseNovelHTML: %v", err)
	}
	...
	if novel.Chapters[0].Title != "chapter 1" || novel.Chapters[0].URL != "https://www.69shuba.com/txt/1/1" {
		t.Fatalf("first chapter = %#v", novel.Chapters[0])
	}
}

func TestNewClientDefaultsToClawreqFetcher(t *testing.T) {
	client := NewClient(nil)
	if _, ok := client.HTMLFetcher.(*ClawreqFetcher); !ok {
		t.Fatalf("HTMLFetcher = %T, want *ClawreqFetcher", client.HTMLFetcher)
	}
}

func TestNewHTTPClientWithOptionsUsesStandardHTTPFetcher(t *testing.T) {
	var called bool
	client := NewHTTPClientWithOptions(fakeHTTPClient(func(req *http.Request) string {
		called = true
		if got := req.Header.Get("Cookie"); got != "cf_clearance=token" {
	...
	if client.HTMLFetcher != nil {
		t.Fatalf("HTMLFetcher = %T, want nil", client.HTMLFetcher)
	}
	if _, err := client.FetchNovel("https://www.69shuba.com/book/1/"); err != nil {
		t.Fatalf("FetchNovel: %v", err)
	}
	if !called {
...
func TestFetchNovelArchiveFetchesChapterContents(t *testing.T) {
	fetcher := &fakeHTMLFetcher{pages: map[string]string{
		"https://www.69shuba.com/book/1.htm": `<!doctype html><html><head><meta charset="utf-8"><meta property="og:novel:book_name" content="book"></head><body><a class="more-btn" href="https://www.69shuba.com/book/1/">完整目录</a><div class="qustime"><ul><li><a href="/txt/1/10"><span>chapter 10</span></a></li></ul></div></body></html>`,
		"https://www.69shuba.com/book/1/":    `<!doctype html><html><body><h1>book</h1><div class="qustime"><ul><li><a href="/txt/1/1"><span>chapter 1</span></a></li><li><a href="/txt/1/2"><span>chapter 2</span></a></li></ul></div></body></html>`,
		"https://www.69shuba.com/txt/1/1":    `<!doctype html><html><body><div class="txtnav"><h1>chapter 1</h1><div id="htmlContent">line one</div></div></body></html>`,
		"https://www.69shuba.com/txt/1/2":    `<!doctype html><html><body><div class="txtnav"><h1>chapter 2</h1><div id="htmlContent">line two</div></div></body></html>`,
	}}
	client := NewClientWithHTMLFetcher(fetcher, "", "")
	archive, err := client.FetchNovelArchive("https://www.69shuba.com/book/1.htm", nil, NovelArchiveOptions{Concurrency: 2})
	if err != nil {
		t.Fatalf("FetchNovelArchive: %v", err)
	}
	if archive.Fetch == nil || archive.Fetch.FullCatalogHTML == "" || len(archive.Chapters) != 2 {
		t.Fatalf("archive = %#v", archive)
	}
	if archive.Chapters[0].Content.Content != "line one" || archive.Chapters[1].Content.Content != "line two" {
		t.Fatalf("chapter contents = %#v", archive.Chapters)
	}
	if !strings.Contains(archive.Chapters[1].ParsedHTML, "line two") {
		t.Fatalf("chapter html = %q", archive.Chapters[1].ParsedHTML)
	}
...
func TestFetchNovelArchiveAllowsPartialChapterFailures(t *testing.T) {
	fetcher := &fakeHTMLFetcher{pages: map[string]string{
		"https://www.69shuba.com/book/1.htm": `<!doctype html><html><head><meta charset="utf-8"><meta property="og:novel:book_name" content="book"></head><body><a class="more-btn" href="https://www.69shuba.com/book/1/">完整目录</a><div class="qustime"><ul><li><a href="/txt/1/10"><span>chapter 10</span></a></li></ul></div></body></html>`,
		"https://www.69shuba.com/book/1/":    `<!doctype html><html><body><h1>book</h1><div class="qustime"><ul><li><a href="/txt/1/1"><span>chapter 1</span></a></li><li><a href="/txt/1/2"><span>chapter 2</span></a></li></ul></div></body></html>`,
		"https://www.69shuba.com/txt/1/1":    `<!doctype html><html><body><div class="txtnav"><h1>chapter 1</h1><div id="htmlContent">line one</div></div></body></html>`,
		"https://www.69shuba.com/txt/1/2":    `<!doctype html><html><body><div class="txtnav"><h1>chapter 2</h1></div></body></html>`,
	}}
	client := NewClientWithHTMLFetcher(fetcher, "", "")
	var reported []ChapterFetchResult
	archive, err := client.FetchNovelArchive("https://www.69shuba.com/book/1.htm", nil, NovelArchiveOptions{
		Concurrency:  2,
		AllowPartial: true,
		OnChapter: func(done int, total int, chapter ChapterFetchResult) {
			reported = append(reported, chapter)
		},
	})
	if err != nil {
		t.Fatalf("FetchNovelArchive: %v", err)
	}
	if len(archive.Chapters) != 2 || archive.Chapters[0].Error != "" || archive.Chapters[1].Error == "" {
		t.Fatalf("chapters = %#v", archive.Chapters)
	}
	if len(reported) != 2 {
		t.Fatalf("reported chapters = %#v", reported)
	}

	_, err = client.FetchNovelArchive("https://www.69shuba.com/book/1.htm", nil, NovelArchiveOptions{Concurrency: 2})
	if err == nil {
		t.Fatal("expected non-partial archive fetch to fail")
	}
}
