...
const defaultImageProxyReferer = BaseURL + "/"

var instagramImageProxyHTTPClient = &http.Client{Timeout: 30 * time.Second}
...
func HandleImageProxy(ctx *gin.Context) {
	...
	resp, err := instagramImageProxyHTTPClient.Do(req)
	if err != nil {
...
func newImageProxyRequest(ctx context.Context, targetURL string, inbound *http.Request) (*http.Request, error) {
	...
	if parsed.Scheme != "http" && parsed.Scheme != "https" {
		return nil, fmt.Errorf("unsupported instagram image URL scheme: %s", parsed.Scheme)
	}
	if !isAllowedImageProxyHost(parsed.Hostname()) {
		return nil, fmt.Errorf("unsupported instagram image host: %s", parsed.Hostname())
	}
...
func setImageProxyHeaders(req *http.Request) {
	...
	req.Header.Set("Referer", defaultImageProxyReferer)
