...
func enable_gsettings_proxy(session linux_proxy_session, ps ProxySettings, port_int int) error {
	...
	// Execute all commands as the login user (with DBUS environment)
	for _, c := range cmds {
	...
	fmt.Println("System proxy successfully enabled (Linux GNOME / Deepin)")
	return nil
}

// DisableProxyInLinux disables the GNOME / Deepin system proxy
func disable_proxy(arg ProxySettings) error {
	...
	fmt.Println("System proxy disabled (Linux)")
	return nil
}
