...
当前 Bridge 桥接/转发服务的原生 JavaScript 源码和 Worker + Pages 部署编排分别位于 `internal/workers/bridge/api/index.js`、`internal/workers/bridge/deploy.go`；`cmd/deploy.go` 只负责读取 CLI 配置、调用 `bridge.Deploy` 和展示结果。
