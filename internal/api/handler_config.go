...
func (c *APIClient) handle_application_config_update(ctx *gin.Context) {
	...
	if c.service_app == nil || c.service_app.Restart == nil {
	...
	restart_token, err := c.service_app.Restart.NewConfirmationToken(config_revision)
	if err != nil {
		result.Err(ctx, 500, "配置已保存，但生成重启确认令牌失败: "+err.Error())
		return
	}
	if err := c.service_app.Restart.Schedule(func(restart_err error) {
		if c.logger != nil {
...
func (c *APIClient) handle_application_restart_status(ctx *gin.Context) {
	...
	confirmation, err := c.service_app.Restart.CheckConfirmation(restart_token, config_revision)
	if err != nil {
		result.Err(ctx, 400, err.Error())
		return
	}
