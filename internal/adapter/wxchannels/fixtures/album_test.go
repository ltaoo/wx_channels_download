...
func TestBuildDownloadTask_FromAlbumJSON(t *testing.T) {
	...
	if len(info.AlbumImages) != len(expected_album.Images) {
		t.Fatalf("AlbumImages length = %d, want %d", len(info.AlbumImages), len(expected_album.Images))
	}
	for i, want := range expected_album.Images {
		assert_equal(t, fmt.Sprintf("AlbumImages[%d]", i), want, info.AlbumImages[i])
	}
	...
	for i, file := range obj.ObjectDesc.Media {
		...
		got_resource := resource_info.DownloadResource
	...
	got_bgm := bgm_info.DownloadResource
...
function onFilename(systemName, meta, task, config) {
  return meta.author + "/" + String(systemName || "").replace(/_$/, "");
}
`), 0644); err != nil {
	...
	if info.Resources[0].UniqueID != expected_album_external_id+"_0" {
		t.Fatalf("DownloadResource.UniqueID = %q, want %q", info.Resources[0].UniqueID, expected_album_external_id+"_0")
	}
	for i := range obj.ObjectDesc.Media {
		...
		if info.Resources[i].DownloadResource.Name != want_resource_name {
			t.Fatalf("DownloadResource[%d].Name = %q, want %q", i, info.Resources[i].DownloadResource.Name, want_resource_name)
		}
	}
	want_bgm_name := expected_album_account_nickname + "/" + expected_album_bgm_name + ".mp3"
	if info.Resources[len(obj.ObjectDesc.Media)].DownloadResource.Name != want_bgm_name {
		t.Fatalf("DownloadResource[bgm].Name = %q, want %q", info.Resources[len(obj.ObjectDesc.Media)].DownloadResource.Name, want_bgm_name)
	}
}
...
func TestBuildDownloadTask_Cover_WithFilenameTemplate(t *testing.T) {
	...
	want_name := expected_album_account_nickname + "/" + expected_album_title + ".jpg"
	if info.Task.Name != want_name {
		t.Fatalf("DownloadTask.Name = %q, want %q", info.Task.Name, want_name)
	}
