...
func TestToContent_EmptyID(t *testing.T) {
	_, err := adapter.ToContent(&scraper.ChannelsObject{})
	if err == nil {
...
func TestToContent_LiveAnchorContactFallback(t *testing.T) {
	...
	content, err := adapter.ToContent(obj)
	if err != nil {
...
func TestToContent_FullConversion(t *testing.T) {
	...
	// Verify ID building
	if id := adapter.BuildContentID(got.ExternalId); id != "wx_channels:14885057406549363320" {
...
func TestToAccount(t *testing.T) {
	...
	if id := adapter.BuildAccountID(got.ExternalId); id != "wx_channels:author" {
...
func TestPlatformID(t *testing.T) {
	if adapter.PlatformID != "wx_channels" {
