...
const fixture_path = "wxchannels/260710/video.json"
...
func int_pointer(value int) *int {
	return &value
}
...
func TestFilenameHookUsesExplicitOrderedDirectories(t *testing.T) {
...
`); err != nil {
	...
	result := hermes.BuildFinalResourceName(hermes.FinalResourceNameInput{
		ResourceName: "original",
		ResourceKind: "video/mp4",
		Hooks:        hooks,
	})
	if result.HookError != nil {
		t.Fatalf("invoke filename hook: %v", result.HookError)
	}
	if got, want := result.Name, "dir1/dir2/part_name.mp4"; got != want {
		t.Fatalf("Name = %q, want %q", got, want)
	}
}

func TestFilenameHookRejectsLegacyStringResult(t *testing.T) {
	hooks := hermes.NewHookManager()
	if err := hooks.Load(`function onFilename() { return "dir/name"; }`); err != nil {
		t.Fatalf("load hook script: %v", err)
	}
	...
	if result.HookError == nil {
		t.Fatal("expected a structured onFilename result error")
	}
	if got, want := result.Name, "original.mp4"; got != want {
...
function onFilename(meta, task, config) {
  return {
    name: meta.filename + "_" + meta.author,
...
`); err != nil {
	...
	want_name := expected_account_nickname + "/" + expected_title + "_" + expected_account_nickname
	...
	if info.Resources[0].Resource.Name != want_name {
		t.Fatalf("Resource.Name = %q, want %q", info.Resources[0].Resource.Name, want_name)
	}
}
...
func TestBuildDownloadTaskWithMultiResource_FromContent(t *testing.T) {
	...
	if info.Resources[0].Resource.Kind != "video/mp4" {
		t.Errorf("Resource.Kind = %q, want %q", info.Resources[0].Resource.Kind, "video/mp4")
	}
	if info.Resources[0].Resource.UniqueID != expected_external_id+"_xWT111" {
		t.Errorf("Resource.UniqueID = %q, want %q", info.Resources[0].Resource.UniqueID, expected_external_id+"_xWT111")
	}
	if info.Resources[0].Resource.Size != 9613487 {
		t.Errorf("Resource.Size = %d, want %d", info.Resources[0].Resource.Size, int64(9613487))
	}
...
func TestBuildDownloadTask_FromContent_WithSubtasks(t *testing.T) {
	...
	resources := []model.DownloadResource{
		{Id: 101, TaskId: int_pointer(task_id), Name: expected_title + ".mp4", Kind: "video/mp4", Size: 9613487, Status: 0, MergeOrder: 0},
		{Id: 102, TaskId: int_pointer(task_id), Name: expected_title + ".jpg", Kind: "image/jpeg", Size: 0, Status: 0, MergeOrder: 1},
		{Id: 103, TaskId: int_pointer(task_id), Name: expected_title + ".mp3", Kind: "audio/mpeg", Size: 0, Status: 0, MergeOrder: 2},
	}
	...
	for i, r := range resources {
		...
		if r.TaskId == nil || *r.TaskId != task_id {
			t.Errorf("%s.TaskId = %v, want %d", sub, r.TaskId, task_id)
		}
	...
	for i := range resources {
		...
		if resources[i].TaskId == nil || *resources[i].TaskId != task_id {
			t.Errorf("chain: resource[%d].TaskId(%v) != %d", i, resources[i].TaskId, task_id)
		}
