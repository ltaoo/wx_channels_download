...
func (mp *MediaProxyWithDecrypt) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if decryptKeyStr != "" {
		...
		if toMP3 == "1" {
			mp.convertWithDecrypt(w, targetURL, decryptKey, 131072)
			return
		}
		mp.decryptOnly(w, r, targetURL, decryptKey, 131072, filename)
		return
	}
	mp.convertOnly(targetURL, w, "mp3")
}

func (mp *MediaProxyWithDecrypt) convertWithDecrypt(w http.ResponseWriter, targetURL string, key uint64, encLimit uint64) {
...
func (mp *MediaProxyWithDecrypt) convertOnly(targetURL string, w http.ResponseWriter, format string) {
	if format != "mp3" {
...
func (ds *DownloadServer) Start() error {
	proxy := NewMediaProxyWithDecrypt()
	http.Handle("/download", withCORS(proxy))
