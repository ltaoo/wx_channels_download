(function(global, factory) {
	typeof exports === "object" && typeof module !== "undefined" ? factory(exports, require("@timeless/timeless"), require("@timeless/kit"), require("@timeless/ui-vm")) : typeof define === "function" && define.amd ? define([
		"exports",
		"@timeless/timeless",
		"@timeless/kit",
		"@timeless/ui-vm"
	], factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, factory((global.Timeless = global.Timeless || {}, global.Timeless.web = {}), global.Timeless, global.Timeless.kit, global.Timeless.ui));
})(this, function(exports, _timeless_timeless, _timeless_kit, _timeless_ui_vm) {
	...
	function connect(app) {
		...
		*/
		function get_system_theme() {
			try {
				if (window.matchMedia(MediaColorSchemeDarkText).matches) return "dark";
				return "light";
			} catch (err) {
				return "light";
			}
		...
		function get_theme() {
			...
			if (!existing_theme || existing_theme === "system") return get_system_theme();
			if (color_schemes.includes(existing_theme)) return existing_theme;
			return "light";
		}
		...
		function set_theme(theme) {
			...
			return _timeless_timeless.Result.Ok(null);
		}
		...
		app.toggleTheme = () => {
			const next = get_theme() === "dark" ? "light" : "dark";
			app.setTheme(next);
			return next;
		};
		const { availHeight, availWidth } = window.screen;
		if (window.navigator.userAgent.match(/iphone/i)) app.safeArea = !![
			"375-812",
			"414-896",
			"428-926",
			"390-844",
	...
	function getProtocols(channel, options) {
		if (typeof options.protocols === "function") return options.protocols(channel);
		return options.protocols;
	}
	function normalizeUrl(channel) {
		if (typeof channel.endpoint !== "string") return _timeless_timeless.Result.Err("ChannelCore.endpoint 必须是字符串");
		const endpoint = channel.endpoint;
		const hostname = channel.hostname || "";
		const location = globalThis.location;
		const hasLocation = !!location?.protocol && !!location?.host;
		const websocketProtocol = hasLocation && location.protocol === "https:" ? "wss:" : "ws:";
		const raw = (() => {
			if (/^wss?:\/\//i.test(endpoint)) return endpoint;
			if (/^https?:\/\//i.test(endpoint)) return endpoint.replace(/^http/i, "ws");
			if (endpoint.startsWith("//")) return `${websocketProtocol}${endpoint}`;
			if (hostname) return [hostname, endpoint].join("");
			if (endpoint.startsWith("/") && hasLocation) return `${websocketProtocol}//${location.host}${endpoint}`;
			return endpoint;
		})();
		const query = channel.query;
		if (!query) return _timeless_timeless.Result.Ok(raw);
		try {
			const base = hasLocation ? location.href : void 0;
			...
			return _timeless_timeless.Result.Ok(url.toString());
		} catch (err) {
			...
			if (!search) return _timeless_timeless.Result.Ok(raw);
			const joiner = raw.includes("?") ? "&" : "?";
			return _timeless_timeless.Result.Ok([
				raw,
				joiner,
				search
			].join(""));
	...
	function installChannel(channel, options) {
		channel.openConnection = () => {
			const binding = bindings.get(channel);
			if (binding) {
				if (binding.socket.readyState === binding.socket.OPEN) return _timeless_timeless.Result.Ok(null);
				binding.socket.close();
				bindings.delete(channel);
			}
			const WebSocketCtor = getWebSocketCtor(options);
			if (typeof WebSocketCtor !== "function") return _timeless_timeless.Result.Err("当前环境缺少 WebSocket");
			const url = normalizeUrl(channel);
			if (url.error) return _timeless_timeless.Result.Err(url.error);
			return new Promise((resolve) => {
				const protocols = getProtocols(channel, options);
				const socket = protocols !== void 0 ? new WebSocketCtor(url.data, protocols) : new WebSocketCtor(url.data);
				...
				socket.onopen = () => {
					if (options.debug) console.log("[provider-web]websocket open", url.data);
					settle(_timeless_timeless.Result.Ok(null));
				};
				socket.onmessage = (event) => {
					const data = decodeMessage(event.data);
					if (options.debug) console.log("[provider-web]websocket message", data);
				...
				socket.onerror = (event) => {
					...
					if (!settled) {
						bindings.delete(channel);
						settle(_timeless_timeless.Result.Err(error));
						return;
					}
					channel.handleError(error);
					if (options.debug) console.log("[provider-web]websocket error", event);
				};
				socket.onclose = (event) => {
					bindings.delete(channel);
					if (!settled) {
						settle(_timeless_timeless.Result.Err(event.reason || "WebSocket closed before open"));
						return;
					}
					channel.handleClose({
						code: event.code,
		...
		channel.postMessage = (data) => {
			const binding = bindings.get(channel);
			if (!binding || binding.socket.readyState !== binding.socket.OPEN) return _timeless_timeless.Result.Err("WebSocket 未连接");
			const payload = encodeMessage(data);
			binding.socket.send(payload);
			if (options.debug) console.log("[provider-web]websocket send", payload);
			return _timeless_timeless.Result.Ok(null);
		};
		channel.closeConnection = (code, reason) => {
			const binding = bindings.get(channel);
			if (!binding) return _timeless_timeless.Result.Ok(null);
			const socket = binding.socket;
			bindings.delete(channel);
			if (socket.readyState === socket.CLOSING || socket.readyState === socket.CLOSED) return _timeless_timeless.Result.Ok(null);
			socket.close(code, reason);
			return _timeless_timeless.Result.Ok(null);
		};
		return channel;
	}
	function scheduleConnect(channel) {
		const run = () => {
			channel.connect();
		};
	...
	function connect$1(channel, options = {}) {
		const autoConnect = options.autoConnect !== false;
		if (!channel) {
			(0, _timeless_kit.onChannelCreated)((ins) => {
				installChannel(ins, options);
				if (autoConnect) scheduleConnect(ins);
			});
			return null;
		}
		installChannel(channel, options);
		if (autoConnect) scheduleConnect(channel);
		return channel;
	}
	function connect$2(vm) {
			...
			return new Promise((resolve) => {
				...
				const pasteHandler = (e) => {
					try {
						...
						if (pastedText) {
							resolve(_timeless_timeless.Result.Ok(pastedText));
							return;
						}
						resolve(_timeless_timeless.Result.Err("读取内容失败"));
					} catch (error) {
						resolve(_timeless_timeless.Result.Err(error));
					} finally {
						cleanup();
					}
				};
				...
				try {
					if (!document.execCommand("paste")) resolve(_timeless_timeless.Result.Err(/* @__PURE__ */ new Error("execCommand失败")));
				} catch (err) {
					resolve(_timeless_timeless.Result.Err(err));
				}
			});
		}
		vm.methods.readText = async function() {
			if (supported) try {
				const text = await navigator.clipboard.readText();
				return _timeless_timeless.Result.Ok(text);
			} catch (err) {
				return readTextFallback();
			}
			return readTextFallback();
		};
		vm.methods.writeText = function(text) {
			const textArea = document.createElement("textarea");
			textArea.value = text;
			document.body.appendChild(textArea);
			textArea.select();
	...
	*/
	...
	utils_default.inherits(CanceledError, AxiosError, { __CANCEL__: true });
	/**
	...
	*/
	...
	function connect$4(store) {
		...
		store.cancel = (id) => {
			...
			if (!matched) return _timeless_timeless.Result.Err("没有找到对应请求");
			requests = requests.filter((r) => r.id !== id);
			matched.source.cancel("主动取消");
			return _timeless_timeless.Result.Ok(null);
		};
	}
	var ownerDocument = globalThis.document;
	function connect$3(history) {
		history.reload = () => {
	...
	function connect$6(store, $input) {
		store.focus = () => {
			$input.focus();
		};
		store.blur = () => {
	...
	var SpeedClassName = "enable-hardware";
	function connectScroll(store, $scroll) {
		function handlePointerDown(e) {
			store.handlePointDown(e);
			const scrollTop = store.getScrollTop();
			if (store.os.pc && scrollTop <= 0) {
				$scroll.addEventListener("mousemove", store.handleMouseMove, { passive: false });
		...
		$scroll.addEventListener("touchmove", store.handleTouchMove, { passive: false });
		...
		let preWinX = 0;
		let preWinY = 0;
		function handleTouchMoveInBounce(e) {
			...
			while (el && el !== document.body && el !== document) {
				...
				if (cls) {
					if (cls.contains("mescroll") || cls.contains("mescroll-touch")) {
						...
						if (!preWinX) preWinX = curX;
						if (!preWinY) preWinY = curY;
			...
			if (isPrevent) (0, _timeless_ui_vm.preventDefault)(e);
		}
		store.refreshRect = () => {
			const { scrollHeight } = $scroll;
			store.setRect({ contentHeight: scrollHeight });
		};
		store.setBounce = (isBounce) => {
			/** 不支持非 iOS 设备 */
			if (!store.os.ios) return;
			if (isBounce === false) window.addEventListener("touchmove", handleTouchMoveInBounce, { passive: false });
			else window.removeEventListener("touchmove", handleTouchMoveInBounce);
		};
		let _scroll_top = 0;
		...
		function getStep(star, end, callback, t = 300, rate = 30) {
			const diff = end - star;
			if (t === 0 || diff === 0) {
	...
	function connectIndicator(store, $indicator) {
		store.hideIndicator = () => {
			$indicator.innerHTML = "";
		};
	...
	function connect$8($video, player) {
		$video.onloadstart = () => {
		...
		$video.onvolumechange = (event) => {
			...
			if (player._curVolume === cur_volume) return;
		...
		$video.onerror = (event) => {
			...
			const msg = (() => {
				...
				if (errorCode === MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED) return /* @__PURE__ */ new Error("不支持的视频格式");
				if (errorCode === MediaError.MEDIA_ERR_DECODE) return /* @__PURE__ */ new Error("视频解码错误");
				if (errorCode === MediaError.MEDIA_ERR_ABORTED) return /* @__PURE__ */ new Error("视频加载中止");
				return /* @__PURE__ */ new Error("unknown");
			})();
	...
	exports.provide_app = connect;
	exports.provide_channel = connect$1;
	exports.provide_clipboard = connect$2;
	exports.provide_history = connect$3;
	exports.provide_http_client = connect$4;
	exports.provide_ui_image = connect$5;
	exports.provide_ui_input = connect$6;
	exports.provide_ui_node = connect$7;
	exports.provide_ui_scroll_view_indicator = connectIndicator;
	exports.provide_ui_scroll_view_scroll = connectScroll;
	exports.provide_ui_video_player = connect$8;
});
