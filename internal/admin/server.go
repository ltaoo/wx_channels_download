...
func (s *AdminServer) handleFavicon(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "image/png")
	w.Header().Set("Cache-Control", "public, max-age=86400")
	http.ServeFile(w, r, "build/winres/icon.png")
}
...
func (s *AdminServer) frontendHandler() http.Handler {
	...
	if s.cfg.Mode == "debug" {
		if _, err := os.Stat(filepath.Join(root, "index.html")); err == nil {
			return frontendserver.New(frontendserver.Options{
				...
				StaticAssetPrefixes: []string{"/lib", "/src", "/public", "/inject"},
	...
	return frontendserver.New(frontendserver.Options{
		...
		EntryPage:           "index.html",
		StaticAssetPrefixes: []string{"/lib", "/src", "/public"},
		NoFallbackPrefixes:  []string{"/api"},
	})
}
