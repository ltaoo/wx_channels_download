...
func (s *AdminServer) handleSandbox(w http.ResponseWriter, r *http.Request) {
	...
	case rest == "session/apply":
		s.handleSandboxSessionApply(w, r, id)
	case rest == "session" || strings.HasPrefix(rest, "session/"):
		s.handleSandboxSessionProxy(w, r, id, sandboxProxyPath(rest, "session"))
	default:
...
func (s *AdminServer) handleSandboxRecord(w http.ResponseWriter, r *http.Request, id string) {
	...
	case http.MethodPatch:
		...
		s.updateSandboxAlias(w, id, req.Alias)
...
func (s *AdminServer) updateSandboxAlias(w http.ResponseWriter, id string, alias string) {
	if err := s.browserMgr.UpdateAlias(id, alias); err != nil {
...
func (s *AdminServer) destroySandbox(w http.ResponseWriter, r *http.Request, id string) {
	if s.browserMgr == nil {
		s.writeError(w, http.StatusInternalServerError, "browser manager not initialized")
		return
	}
...
func (s *AdminServer) handleRefreshSandbox(w http.ResponseWriter, r *http.Request, id string) {
	if r.Method != http.MethodPost {
		w.WriteHeader(http.StatusMethodNotAllowed)
		return
	}
	rec, err := s.browserMgr.Refresh(r.Context(), id)
	if err != nil {
		s.writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
...
func (s *AdminServer) handleSandboxSessionApply(w http.ResponseWriter, r *http.Request, id string) {
	if r.Method != http.MethodPost {
		w.WriteHeader(http.StatusMethodNotAllowed)
		return
	}
...
func (s *AdminServer) handleSandboxSessionProxy(w http.ResponseWriter, r *http.Request, id string, proxyPath string) {
	if !validSandboxTicket(id, sandboxTicketFromRequest(r)) {
		writeJSON(w, http.StatusUnauthorized, map[string]any{"error": "invalid ticket"})
		return
	}
	rec, ok := s.browserRecord(id)
	if !ok || rec.Endpoint == nil {
		writeJSON(w, http.StatusNotFound, map[string]any{"error": "sandbox not found"})
		return
	}
	if rec.Status != browsermgr.StatusRunning && rec.Status != browsermgr.StatusIdle && rec.Status != browsermgr.StatusBusy {
		writeJSON(w, http.StatusNotFound, map[string]any{"error": "sandbox not running"})
		return
	}
	targetText := firstNonEmpty(rec.Endpoint.SessionURL, rec.Endpoint.DesktopURL)
	if targetText == "" {
		writeJSON(w, http.StatusNotFound, map[string]any{"error": "sandbox has no session endpoint"})
		return
	}
	targetURL, err := url.Parse(targetText)
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]any{"error": "invalid session endpoint"})
		return
	}
...
func sandboxTicketFromRequest(r *http.Request) string {
...
func sandboxProxyPath(rest string, prefix string) string {
	rest = strings.Trim(rest, "/")
	prefix = strings.Trim(prefix, "/")
	if rest == prefix {
		return "/"
	}
	return "/" + strings.TrimPrefix(strings.TrimPrefix(rest, prefix), "/")
}

func joinProxyPath(base string, rel string) string {
