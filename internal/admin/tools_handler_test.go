...
func TestAdminToolsRoutes(t *testing.T) {
	...
	sessionReq := httptest.NewRequest(http.MethodPost, "/api/v1/sandboxes/"+createResp.Data.ID+"/session/apply", bytes.NewBufferString(`{"ttl_sec":60}`))
	sessionReq.Header.Set("Content-Type", "application/json")
	sessionRec := httptest.NewRecorder()
	handler.ServeHTTP(sessionRec, sessionReq)
	if sessionRec.Code != http.StatusOK {
		t.Fatalf("session apply code = %d body=%s", sessionRec.Code, sessionRec.Body.String())
	}
	...
	if sessionResp.Code != 0 || sessionResp.Data.Ticket == "" {
		t.Fatalf("session apply response = %#v", sessionResp)
	}

	updateReq := httptest.NewRequest(http.MethodPost, "/api/v1/sandboxes/"+createResp.Data.ID, bytes.NewBufferString(`{"_method":"PATCH","alias":"renamed"}`))
	updateReq.Header.Set("Content-Type", "application/json")
	updateRec := httptest.NewRecorder()
	handler.ServeHTTP(updateRec, updateReq)
	if updateRec.Code != http.StatusOK {
		t.Fatalf("update sandbox code = %d body=%s", updateRec.Code, updateRec.Body.String())
	}

	detailReq := httptest.NewRequest(http.MethodGet, "/api/v1/sandboxes/"+createResp.Data.ID, nil)
	detailRec := httptest.NewRecorder()
	handler.ServeHTTP(detailRec, detailReq)
	if detailRec.Code != http.StatusOK {
		t.Fatalf("detail sandbox code = %d body=%s", detailRec.Code, detailRec.Body.String())
	}
	...
	if detailResp.Code != 0 || detailResp.Data.Alias != "renamed" {
		t.Fatalf("detail sandbox response = %#v", detailResp)
	}

	deleteReq := httptest.NewRequest(http.MethodPost, "/api/v1/sandboxes/"+createResp.Data.ID, bytes.NewBufferString(`{"_method":"DELETE"}`))
	deleteReq.Header.Set("Content-Type", "application/json")
	deleteRec := httptest.NewRecorder()
	handler.ServeHTTP(deleteRec, deleteReq)
	if deleteRec.Code != http.StatusOK {
		t.Fatalf("delete sandbox code = %d body=%s", deleteRec.Code, deleteRec.Body.String())
	}
}
