...
func TestBrowseHistoryListOrdersRepeatedVisitsByLatestUpdate(t *testing.T) {
	...
	if err := (&model.BrowseHistory{
		...
		VisitedTimes: 1,
		ExternalId:   "old_then_latest",
		Title:        "old then latest",
	...
	if histories[0].ExternalId != "old_then_latest" {
...
func TestBrowseHistoryListPlatforms(t *testing.T) {
	...
	for _, history := range []model.BrowseHistory{
		{PlatformId: "wxchannels", VisitedTimes: 1, ExternalId: "video_1", Timestamps: model.Timestamps{CreatedAt: 1000, UpdatedAt: 1000}},
		{PlatformId: "wxmp", VisitedTimes: 1, ExternalId: "article_1", Timestamps: model.Timestamps{CreatedAt: 2000, UpdatedAt: 2000}},
		{PlatformId: "douyin", VisitedTimes: 1, ExternalId: "douyin_1", Timestamps: model.Timestamps{CreatedAt: 3000, UpdatedAt: 3000}},
	} {
	...
	histories, err := NewBrowseService(db).ListPlatforms([]string{"wxchannels", "wxmp"}, nil)
	if err != nil {
	...
	if histories[0].ExternalId != "article_1" || histories[1].ExternalId != "video_1" {
